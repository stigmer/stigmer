 ...
 * and its save is an upsert (`store/sqlite/store.ts`), so a
 * lookup-then-insert on a scan could never close the first-login race. Two
 * replicas deriving the same id converge by primary key with no migration
 * and no second copy of any field. The hash is one-way, so an issuer
...
export function accountIdFor(idpId: string): string {
  ...
  return derivedId(ACCOUNT_ID_PREFIX, idpId);
}

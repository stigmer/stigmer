 ...
 * creates the session even under a denying run-gate authorizer — the
 * organization's own create permission admitted it in the Authorize step;
...
describe("session-create run gate (composed server)", () => {
  ...
  it("makes no check for the built-in assistant (empty spec) and creates the session", async () => {
    decision = { kind: "deny", reason: "" };
    gate.runGateChecks.length = 0;
    const before = await sessionCount();
    ...
    // No instance, no blueprint to spend, no target: the gate is silent
    // and the denying authorizer is never asked.

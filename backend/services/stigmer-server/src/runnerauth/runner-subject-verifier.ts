 ...
 * token AS THE HUMAN WHO ASKED: for a run, the person the execution row's
 * creator stamp names, for as long as that run lives; for an MCP
 ...
 * binding; the server resolves the person from the row the way the
 ...
 * the decrypt lane falls closed on, one rule in one home.
 *
 ...
 * HMAC, a present `exp` in the past, a binding that names no kind or no
 * row (the run is not the caller's to learn about, so never NOT_FOUND),
 * a clockless token bound to a connect — and the liveness sentence for a
 * genuine credential whose run is over or was created by nobody this
 * server recognizes. A store fault propagates as the fault it is: the
 * chassis maps it INTERNAL (pipeline/interceptors/auth.ts), so an outage
 * never reads as a bad credential.
 */
...
import { bindsARun, loadBoundExecution } from "./bound-execution.js";
...
export function newRunnerSubjectIdentityVerifier(
  ...
  return {
    ...
    async verify(token: string): Promise<CallerIdentity | null> {
      ...
      if (!isClockedToken(token) && !bindsARun(execution.kind)) {
        throw invalidCredential();
      }
      if (!execution.live) {
        throw notLive();

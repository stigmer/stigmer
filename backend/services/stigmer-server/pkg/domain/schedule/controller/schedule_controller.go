...
// (next fire time, failure streak, auto-pause) lives in status, whose
// sole writer is the scheduling runtime — a declarative apply preserves
// status verbatim (the AgentChannel decision-004 posture), so a routine
// manifest apply can never reset a failure streak or un-pause an
// auto-paused schedule.
//

...
// 1. ResolveSlug - Resolve slug (must be before validation, for fallback slug lookup)
// 2. ValidateFieldConstraints - Validate proto field constraints
...
func (c *AgentExecutionController) buildUpdatePipeline() *pipeline.Pipeline[*agentexecutionv1.AgentExecution] {
	return pipeline.NewPipeline[*agentexecutionv1.AgentExecution]("agent-execution-update").
		AddStep(steps.NewResolveSlugStep[*agentexecutionv1.AgentExecution]()).         // 1. Resolve slug (must be before validation, for fallback)

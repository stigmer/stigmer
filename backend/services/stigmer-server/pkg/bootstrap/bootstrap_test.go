...
func TestBootstrapper_Run_Success(t *testing.T) {
	...
	// Verify agents were applied (seedpack has agent-creator, mcp-server-creator, skill-creator, sorted alphabetically)
	assert.Len(t, agentClient.ApplyCalls, 3)
	...
	assert.Equal(t, "mcp-server-creator", agentClient.ApplyCalls[1].GetMetadata().GetName())
	assert.Equal(t, "local", agentClient.ApplyCalls[1].GetMetadata().GetOrg())
	assert.Equal(t, "skill-creator", agentClient.ApplyCalls[2].GetMetadata().GetName())
	assert.Equal(t, "local", agentClient.ApplyCalls[2].GetMetadata().GetOrg())
	...
	mcpServerCreatorAgentState, err := store.GetBootstrapState(ctx, KeyAgentPrefix+"mcp-server-creator")
	require.NoError(t, err)
	assert.Contains(t, mcpServerCreatorAgentState, KeyAppliedPrefix)

...
// GetByReference retrieves a session by ApiResourceReference (slug-based lookup) using the pipeline framework
//
// This implements the GetByReference operation pattern:
// 1. ValidateProto - Validate input ApiResourceReference
// 2. LoadByReference - Load session by slug (with optional org filtering)
//
// Pipeline (Stigmer OSS - simplified from Cloud):
// - ValidateProto: Validates buf.validate constraints on ApiResourceReference
// - LoadByReference: Queries sessions by slug, handles org-scoped and platform-scoped lookups
//
...
// - If ref.org is provided: queries sessions in that org with matching slug
// - If ref.org is empty: queries platform-scoped sessions with matching slug
...
func (c *SessionController) GetByReference(ctx context.Context, ref *apiresource.ApiResourceReference) (*sessionv1.Session, error) {
...
func (c *SessionController) buildGetByReferencePipeline() *pipeline.Pipeline[*apiresource.ApiResourceReference] {
	// api_resource_kind is automatically extracted from proto service descriptor
	// by the apiresource interceptor and injected into request context
	return pipeline.NewPipeline[*apiresource.ApiResourceReference]("session-get-by-reference").
		AddStep(steps.NewValidateProtoStep[*apiresource.ApiResourceReference]()). // 1. Validate input
		AddStep(steps.NewLoadByReferenceStep[*sessionv1.Session](c.store)).       // 2. Load by slug
		Build()
}

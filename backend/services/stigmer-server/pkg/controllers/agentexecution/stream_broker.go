...
func NewStreamBroker() *StreamBroker {
	return &StreamBroker{
		subscribers: make(map[string][]chan *agentexecutionv1.AgentExecution),
	}
}
...
//
// Returns:
// - A channel that receives AgentExecution updates
func (b *StreamBroker) Subscribe(executionID string) chan *agentexecutionv1.AgentExecution {
	b.mu.Lock()
	defer b.mu.Unlock()

	// Create buffered channel to prevent blocking broadcasts
	// Buffer size of 100 allows bursts of updates without blocking
	ch := make(chan *agentexecutionv1.AgentExecution, 100)
...
func (b *StreamBroker) Unsubscribe(executionID string, ch chan *agentexecutionv1.AgentExecution) {
	b.mu.Lock()
	defer b.mu.Unlock()
...
func (b *StreamBroker) Broadcast(execution *agentexecutionv1.AgentExecution) {

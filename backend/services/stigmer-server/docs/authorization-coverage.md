...
**Conformance coverage of the ruled divergences (2026-09-04)** — `test/conformance/src/suites/direct-handler-authorization.conformance.test.ts` pins the outsider contract of the 19 evaluated methods, and three of its arms sit exactly on the Java-edition divergences recorded above (getVersion never evaluated — stigmer-cloud#562; initiateOAuthConnect precondition-before-authorization; PERMISSION_DENIED instead of the uniform NOT_FOUND for unknown ids on the authorize-first family). Both editions serve behind the one `cloud` target, so the suite declares which contract it is asserting via `STIGMER_CONFORMANCE_DIRECT_HANDLER_AUTHZ_CONTRACT` — `java-baseline` (the default; the nightly hermetic Java lane) or `annotation-enforced` (composition readouts). Each contract is enforced strictly on its side, the getVersion arm skips under the Java baseline (there is no refusal to pin until #562 closes at the cutover), and the unknown-id arm observes all six lanes before asserting so a divergence shows as the whole table, not the first lane. The mechanism is the `STIGMER_CONFORMANCE_BILLING_DENIAL_CONTRACT` precedent (billing-denial suite); both knobs retire with the Java target at R1.

/**
 * Heartbeat client for the cursor-runner bidi stream.
 *
 ...
 * these heartbeats for liveness detection and multi-process sandbox
...
export function startHeartbeat(config: Config): () => void {
  ...
  async function runStream(): Promise<void> {
    ...
    // Async generator that yields heartbeats on an interval.
    // The generator is consumed by the bidi stream as its request source.
    async function* heartbeatGenerator(): AsyncIterable<RunnerStreamClientMessage> {
      // Send the first heartbeat immediately to authenticate the stream
    ...
    // Consume server messages (commands) in the background.
    // For now we only log; command handling is a future enhancement.
    ...
    for await (const msg of responses) {
      if (msg.message.case === "commandRequest") {
        console.log(
          `[heartbeat] Received command request: ${msg.message.value.command.case ?? "unknown"}`,
        );
      }
    }
  }
  ...
  console.log(`[heartbeat] Started for runner ${config.runnerId} (process_type=cursor)`);

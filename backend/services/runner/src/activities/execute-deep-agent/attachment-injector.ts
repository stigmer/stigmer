 ...
 * validated and decompressed by `shared/attachment-zip.ts` (the #567 guards:
 * central-directory parsing, path traversal, zip bombs, declared-size
 * re-enforcement), which the runtime's attachment resolver shares; this
 * module only writes what those guards admit, through its workspace
 * backend.
 *
...
} from "../../shared/attachment-zip.js";

// The ZIP guards live in `shared/attachment-zip.ts` since S3 M1 (the
// runtime's attachment resolver extracts through them); this module keeps
// its workspace-backend write loop until M2b retires it with its
// orchestrator. The guard's error is re-exported for this module's tests.
export { AttachmentValidationError } from "../../shared/attachment-zip.js";

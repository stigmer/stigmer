 ...
 *
 * The one module that knows the SDK's wire shape:
 * the shared `TranscriptBuilder` folds what comes out of here and nothing of
 * the engine reaches it otherwise. Until #1097 the Cursor harness folded its
 ...
 * write mid-persist would land on a row the offload is replacing, so this
 * translator never hands a delta's events to anyone
 ...
 * `tool_started` that announces it. Because the fold is later than
 ...
 * the fold's moment. Rows are CREATED by the stream alone: the
 * delta channel amends (output, completion), never announces.
 *
 * WHAT THIS TRANSLATOR ANSWERS, per event:
 *
 *   - The MESSAGE BOUNDARY. Cursor has one `run_id` per `send()`,
 ...
 *     identity token the hook and the grants already share, the
 ...
 *   - The `task` SDK event → `system_note`; a todo write is an
 *     ordinary tool call the builder keys on `ToolKind.TODO` and projects;
 *     `system`, `status`, `user`, `request` and `usage` carry no
...
export class CursorTranslator {
  ...
  observeDelta(update: InteractionUpdate): void {
    switch (update.type) {
      case "tool-call-started":
        ...
        // first stream event); the delta only tells us which shell an
        // output delta that names no call belongs to.
        if (update.toolCall.type === "shell") this.lastShellCallId = update.callId;
        return;
      case "shell-output-delta": {
        // Right by the SDK's type and exercised by the scripted double; the
        // live run behind #1097 (SDK 1.0.31) sent no `shell-output-delta` for
        // a 1.2 s or a 4 s command, so until the SDK sends one this arm has
        // no live evidence of its own.
      ...
      case "tool-call-completed": {
        // The completion, at the instant it was observed, with the
        // OUTCOME the typed delta carries (since #1097): a `result.status`
        // of `error` is a failure, not a completion — until then the enricher
        ...
        // result, and the builder's upsert merges the two; an
   ...
   * identity token), in which case the seeded id — aliased for every

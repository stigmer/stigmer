...
These are orthogonal. The **desktop runner** is the canonical case where they diverge: it executes **locally** (`MODE=local`, so local-path workspaces work) yet still routes Cursor traffic and artifact uploads **through the proxy**. Coupling the two — forcing "proxy implies cloud" — is exactly what previously broke local-path sessions, which is why the env path (`MODE`) and the manager factory (`executionMode`) treat them separately.

> **Current limitation — static library factory.** The decoupling holds for the environment variable (`MODE`) and for `createStigmerRunnerManager` (its `executionMode` option). It does **not** yet hold for the static `createStigmerRunner` library factory, which derives `mode` from whether `proxyEndpoint` is set (`proxy ⇒ cloud`, otherwise `local`) and exposes no `executionMode` option. If you embed `createStigmerRunner` directly and need local execution with proxy transport, that combination is currently not expressible through its options; use manager mode (or the environment-driven CLI) for that case. This asymmetry is a known follow-up.
...
- **[Manager-mode IPC protocol](docs/ipc-protocol.md)** — the line-delimited JSON command/response contract and its versioned handshake (current protocol version `1`).
- **Embedding integration guide** — building a desktop client (local execution) and a web client (cloud execution) (forthcoming).
- **SDK `RunnerAdapter` and `executionTarget`** — the local-execution surfaces in `@stigmer/react` and `@stigmer/sdk` (forthcoming).

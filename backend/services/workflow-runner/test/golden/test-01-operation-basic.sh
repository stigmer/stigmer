...
# Test script for Golden Test 01: Basic Operation State
# Tests basic operation state with simple task execution

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
WORKFLOW_FILE="$SCRIPT_DIR/01-operation-basic.yaml"
GRPC_HOST="${GRPC_HOST:-localhost:9090}"

# Generate unique execution ID
EXECUTION_ID="operation-basic-$(date +%s)"

echo "======================================"
echo "Golden Test 01: Basic Operation State"
echo "======================================"
...
grpcurl -plaintext -d "{
  ...
  \"metadata\": {
    ...
    \"description\": \"Test basic operation state\"
  }
...
echo "  1. Initialize workflow with workflow_started=true"
echo "  2. Set message and status in hello state"
echo "  3. Finalize with workflow_completed=true"
echo ""

...
That's it. Three lines. If `TimeSourceProvider` is present, the step index is derived from the provided time. If not, the timer-based index is used. The rest of the component — the render prop, the caption, the content — is identical in both paths.

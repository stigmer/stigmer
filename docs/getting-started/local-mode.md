...
## Project Configuration (Stigmer.yaml)

Every Stigmer project requires a `Stigmer.yaml` file that defines project metadata.

### Required Configuration

**Minimal `Stigmer.yaml`**:
...
| `runtime` | **Yes** | SDK runtime (`go` for Go SDK) | - |
| `version` | No | Project version (semver) | - |
| `description` | No | Project description | - |
| `main` | No | Entry point file for resource definitions | `main.go` |
| `organization` | No | Override organization ID (otherwise uses CLI context) | - |

### Filename Convention

**Use `Stigmer.yaml` (capital S)** - follows Pulumi convention (`Pulumi.yaml`).

The CLI also accepts `stigmer.yaml` (lowercase) for backwards compatibility, but new projects should use the capitalized version.

### What Stigmer.yaml Doesn't Contain

**Resources are defined in code, not in Stigmer.yaml!**

Unlike other tools where you define resources in YAML, Stigmer uses **Infrastructure as Code** with the Go SDK:

```go
// main.go - Resource definitions in code
package main
...
func main() {
    ...
    // Resources auto-discovered when you run: stigmer apply
}
```

When you run `stigmer apply`, it:
1. Reads `Stigmer.yaml` for project metadata
2. Executes your `main.go` to discover resources
3. Deploys/updates those resources

### Generated by `stigmer new`

When you use `stigmer new`, it automatically generates a complete `Stigmer.yaml` with all required fields:

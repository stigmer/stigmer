...
1. **Add the package.** `npm install @stigmer/runner`. The package ships the
   runner's `dist/` and the runtime dependencies the subprocess needs.
2. **Copy it into your app's resources at build time.** Stage the installed
   package (its `dist/` and resolved `node_modules`) into a resources folder,
   then have your packager bundle that folder into the installer. In Tauri,
   declare it under `bundle.resources` in `tauri.conf.json`:
...
3. **Point the host at the bundled entry.** Set `runner_entry` (the Rust snippet
   above) to the staged path — `resources/runner/dist/main.js` — resolved
   relative to the app's resource directory at runtime.

<Callout type="info">
  The Stigmer desktop app symlinks `resources/runner` to the in-repo runner
  (`backend/services/runner`) because it lives inside the monorepo. An external
  embedder fills the same folder from the published `@stigmer/runner` package
  instead — the Tauri `bundle.resources` config is identical either way.
</Callout>

...
Your `RunnerAdapter` then delegates to this manager: `onSessionOpened` calls
`manager.addSession`, `onSessionClosed` calls `manager.removeSession`, and so
on.
...
driver. Use the
[`stigmer-runner-host`](https://github.com/stigmer/stigmer/blob/main/crates/stigmer-runner-host)
crate. It spawns the runner, performs the versioned `ready` handshake, and
exposes the full lifecycle as an async API:

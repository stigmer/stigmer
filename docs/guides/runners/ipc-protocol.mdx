...
- **Rust host crate** —
  [`crates/stigmer-runner-host`](https://github.com/stigmer/stigmer/tree/main/crates/stigmer-runner-host).
  The packaged, reusable driver (`RunnerHost`); the desktop app consumes it. It
...
- **Conformance harness (TypeScript)** —
  [`test/conformance/src/harness/runner-manager-process.ts`](https://github.com/stigmer/stigmer/blob/main/test/conformance/src/harness/runner-manager-process.ts).
  Drives the protocol **synchronously**: after each command it awaits the
  matching response. This is the strict request/response form of the contract,
  and it imports the runner's `Ipc*` types directly rather than mirroring them.
...
The protocol is defined in three places that must change together:
...
| `crates/stigmer-runner-host/src/protocol.rs`  | Hand-maintained Rust mirror (host crate; desktop consumes it)      |

TypeScript hosts import the `Ipc*` types instead of mirroring them (the
conformance harness does). Golden fixtures bind the Rust mirror to the canonical
TypeScript so drift fails a test instead of slipping through:

- `make gen-ipc-fixtures` serializes one representative instance of every
  message to `backend/services/runner/fixtures/ipc-protocol.generated.json`
  (with a copy vendored into the Rust crate). The samples are type-checked
  against the `ipc-protocol.ts` interfaces, so a renamed or retyped field fails
  to compile.
- The Rust crate runs a conformance test that asserts its types serialize to,
  and parse from, those exact fixtures.
- `make check` runs `gen-ipc-fixtures-check`, which fails if the committed
  fixtures are stale, plus the TypeScript and Rust tests.

When you change the protocol:

1. Edit `ipc-protocol.ts` and update the matching sample in
   `ipc-protocol-fixtures.ts`.
2. Run `make gen-ipc-fixtures` and commit the regenerated fixtures.
3. Update the Rust mirror until its conformance test passes.
4. Update this page.
5. Bump `IPC_PROTOCOL_VERSION` if the change is breaking.

The Go and Rust structs carry a comment pointing back to this page.

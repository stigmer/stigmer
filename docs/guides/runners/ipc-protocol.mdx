...
- **Rust host crate** —
  [`crates/stigmer-runner-host`](https://github.com/stigmer/stigmer/blob/main/crates/stigmer-runner-host).
...
The protocol is defined in four places that must change together:
...
Golden fixtures bind the two code mirrors to the canonical TypeScript so drift
fails a test instead of slipping through:
...
- `make gen-ipc-fixtures` serializes one representative instance of every
  ...
  to compile.
- The Rust crate and the Go harness each run a conformance test that asserts
  their types serialize to, and parse from, those exact fixtures.
- `make check` runs `gen-ipc-fixtures-check`, which fails if the committed
  fixtures are stale, plus the TypeScript and Rust tests. The Go test runs with
...
3. Update the Rust and Go mirrors until their conformance tests pass.
4. Update this page.

...
- The `client_id` is safe for configuration files, logs, and client-side code —
  it is not a secret.
- Anyone who can create a PlatformClient in your Organization (its
  administrators) can mint a token for any `user_id` in it. Treat PlatformClient
  administration as you treat access to your users' accounts.

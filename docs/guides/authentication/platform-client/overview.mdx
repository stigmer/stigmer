...
mint user-scoped JWTs, and pass those tokens to the React SDK. It works the same
on Stigmer Cloud and on a self-hosted server with sign-in configured.
...
<Callout type="info">
  **Looking for a different auth method?** If your users already authenticate
  ...
  key](/docs/sdk/overview#authentication). See the [authentication
...
<ScenarEmbed
  id="platform-client-token-flow"
  title="PlatformClient token flow walkthrough"
/>
...
- A Stigmer server that authenticates its callers: Stigmer Cloud, or a
  ...
  verify them.
- A PlatformClient created in the Console (Settings > Platform Clients) by an
  admin of your organization.
...
By default, `mintUserToken` fails with `FAILED_PRECONDITION` if the user doesn't
have a Stigmer Identity Account. Enable automatic provisioning on the
PlatformClient to create accounts on first encounter:
...
- **`auto_grant_on_org: true`** — grants a role on your organization
  immediately.
- **`auto_grant_role`** — the role to grant (default: `viewer`). `owner` is
  refused: organization ownership is always assigned explicitly.

These settings apply when an account is first provisioned. An account that
already exists keeps the roles and profile it has: changing the settings later,
or passing a different email or name in a later mint, does not update it.
...
| `FAILED_PRECONDITION` | User doesn't exist, automatic provisioning off                                           | Enable `auto_provision_accounts` or pre-create the account.                                         |
| `FAILED_PRECONDITION` | Client secret expired                                                                    | Extend or clear the PlatformClient's expiry in the Console; rotating alone does not extend it.      |
| `FAILED_PRECONDITION` | The server trusts every request                                                          | Configure sign-in on the server (`STIGMER_OIDC_ISSUER`).                                            |
| `INVALID_ARGUMENT`    | `org_id` names an organization other than the PlatformClient's                           | Leave `org_id` empty, or pass the PlatformClient's own organization.                                |
| `UNAUTHENTICATED`     | An API call bearing a token whose PlatformClient was deleted                             | Mint a new token from an active PlatformClient.                                                     |
| `PERMISSION_DENIED`   | A browser API call bearing the minted token came from an origin not in `allowed_origins` | Add the requesting origin to the PlatformClient's `allowed_origins`.                                |
...
- Secret rotation invalidates the old secret immediately. There is no grace
  period. Tokens already minted stay valid until they expire (15 minutes);
  delete the PlatformClient to revoke them on their next request.
- The `client_id` is safe for configuration files, logs, and client-side code —
  it is not a secret.
- Anyone who can create a PlatformClient in your organization (its admins) can
  mint a token for any `user_id` in it. Treat PlatformClient administration as
  you treat access to your users' accounts.
...
- Use `allowed_origins` to restrict which browser origins can use tokens minted
  by this PlatformClient. Enforcement happens on the browser's API calls, not on
  `mintUserToken` (minting is server-to-server and carries no browser Origin):
  with a non-empty list, requests whose `Origin` header is unlisted are refused
  `PERMISSION_DENIED`; an empty list disables the check. Requests without an
  `Origin` header always pass — this is a browser-context defense-in-depth
  control against a leaked token being replayed from another site, and the
  `client_secret` remains the primary control.
...
`FAILED_PRECONDITION` naming the user means the user doesn't have a Stigmer
Identity Account and automatic provisioning is disabled. Enable
`auto_provision_accounts` on the PlatformClient or create the account before
calling `mintUserToken`.

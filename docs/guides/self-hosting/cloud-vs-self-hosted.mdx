...
- **Two packagings, no operator.** Docker Compose for one server and a
  [Helm chart](./kubernetes) for Kubernetes; there is no operator for the stack
  itself, and nothing in it needs one.

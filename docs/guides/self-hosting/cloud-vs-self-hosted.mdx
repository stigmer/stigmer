...
| Authentication                                   | Your OIDC provider, API keys and PlatformClient                               | Hosted login, SSO/SAML, identity federation, PlatformClient |
| Organizations                                    | One team's — the server does not stop a signed-in person creating more        | As many as you need                                         |
| Permissions                                      | Owner, admin, member and viewer roles per Organization; Private stays private | Fine-grained, per-resource roles and sharing                |
...
- **Authentication on your terms.** Point the server at any OIDC provider you
  control — Keycloak, Okta, Auth0, Dex — mint API keys for machines, and let
  your product's backend mint tokens for its own users with a
...
- **One Organization, one team.** Everyone who signs in joins it as a member.
  Owners and admins author Agents, Workflows, Skills and MCP Servers; members
  run them, and schedule, share or connect an Agent to a channel only when they
  may edit it; each person's Sessions, Environments and API keys are their own.
  Roles are per Organization; there is no per-resource sharing. The server does
  not stop a signed-in person from creating a second Organization, but one is
  the shape self-hosting is built for.
...
- **Identity federation.** Bring users from many identity providers with
  [identity federation](/docs/guides/authentication/federation/overview) — trust
  relationships that let your platform's users reach Stigmer without managing
  credentials themselves.
- **Channels.** Agents that talk to your users
  [in Slack and WhatsApp](/docs/guides/channels/connect-slack) — webhook
  delivery, provider apps, and outbound messaging are cloud infrastructure.
...
One honest caveat before you commit to self-hosting today:

- **Two production packagings, no operator.** [Docker Compose](./docker-compose)
  for one server and a [Helm chart](./kubernetes) for Kubernetes run Stigmer for
  real; `stigmer up` and the [all-in-one image](./all-in-one) are for building
  and trying it on a laptop. There is no operator for the stack itself, and
  nothing in it needs one.

## Next step

Self-hosting starts with choosing the shape that fits your team.

<Cards>
  <Card title="Self-host Stigmer" href="/docs/guides/self-hosting/overview">
    Pick a shape — your laptop, one server with Docker Compose, or Kubernetes
    with the Helm chart — and run your first Agent on it.
  </Card>

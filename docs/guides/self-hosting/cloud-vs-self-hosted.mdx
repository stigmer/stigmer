...
- **One Organization, one team.** Everyone who signs in joins it as a member.
  Owners and admins decide who may author Agents, Workflows and Skills; each
  person's Sessions, Environments and API keys are their own. Roles are per
  Organization; there is no per-resource sharing. The server does not stop a
  signed-in person from creating a second Organization, but one is the shape
  self-hosting is built for.

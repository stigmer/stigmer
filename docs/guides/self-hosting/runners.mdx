...
<Callout type="warn">
  **At 3.17.0 the shipped stack can't exercise this.** The published server
  image carries no `docker` CLI and the compose file mounts no Docker socket, so
  the `docker` driver fails at its first Session. And the two container drivers
  hand the per-Session runner no artifact path, so attachments a run needs don't

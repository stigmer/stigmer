...
create an Organization: a fresh stack has none yet. Leave it there; the next
steps bootstrap one from the CLI. Download links the platform mints point at
port 7235.

### Connect the CLI

Tell the CLI where the stack is, as a named backend, then switch to it:
...
<Callout type="warn">
  **Run this step before you turn authentication on.** The default Plugins are
  ...
  stack that turned authentication on first can't be bootstrapped, and a
  bootstrapped stack can't take a newer catalogue
...
stack, end to end. `stigmer list agents` shows both: yours, and the `assistant`
Agent the bootstrap installed.

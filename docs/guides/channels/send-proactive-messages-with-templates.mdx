...
nothing to import. To see the list as Stigmer sees it, open the agent's
**Channels** tab in the console and pick **Templates** from the WhatsApp
channel's card menu. Each template appears with its approval status and its full
text, straight from WhatsApp.
...
- **A template is approved but the Agent never uses it.** Open **Templates** on
  the channel's card in the console. An approved template that Stigmer cannot
  send — say one with a video header or a dynamic-link button — shows the exact
  reason there. The Agent's own list leaves such templates out, so this view is

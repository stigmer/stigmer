...
| **Per-call overhead**   | Minimal (your prompt only)                                           | ~10k tokens of system context per call                     |
...
The harnesses have different cost profiles beyond the commission percentage.
Native benefits from Anthropic prompt caching (up to 78% savings on repeated
prefixes), while cursor carries a fixed ~10k token overhead per call from its
agent tooling.

For multi-turn sessions, native's cost advantage compounds with each turn as the
cached prefix is reused. For coding tasks that require cursor's built-in tools,
the overhead is justified by the capability it provides.

See the [cost optimization guide](/docs/guides/runners/cost-optimization) for a
detailed decision matrix, caching economics, and practical tips for reducing
spend.

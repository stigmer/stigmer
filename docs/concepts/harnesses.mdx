...
- **Native** — Stigmer's built-in engine: the deepagents engine on LangGraph.js,
  running in TypeScript inside the Stigmer runner.
...
The Cursor harness uses the Cursor SDK as the execution engine. Instead of the
Stigmer runner's own engine processing the Agent's turns, the Cursor Agent
...
| **Tool capabilities**       | MCP servers you configure + built-in web fetch                        | MCP servers + Cursor's built-in coding tools                                 |
...
| **Engine context per call** | Stigmer's system prompt and tool schemas, cached after the first call | Cursor's own engine context, cached at Cursor's discretion                   |
| **Prompt caching**          | Full control (explicit breakpoints)                                   | Automatic and opaque                                                         |
| **Latency**                 | Lower on a fresh Session                                              | Higher on a fresh Session (agent start-up); close to native on a resumed one |
...
One [Runner](/docs/concepts/runners) serves both harnesses. The Harness is a
property of the Session and travels with each execution on the same task queue;
the runner reads it and hands the work to the matching runtime. You don't need
separate runner processes, or separate queues, for each Harness.
...
from three places: the Cursor Token Rate, engine context per call, and prompt
caching.
...
- **Engine context per call** — both harnesses send an engine preamble with
  every call, and neither is your prompt alone. Native sends Stigmer's system
  prompt and tool schemas; Cursor sends its own engine context, as the Cursor
  SDK reports usage. The two are comparable in size, so the cost gap between
  them is not token volume.
- **Prompt caching** — native exposes explicit cache breakpoints, so a Session's
  ...
  automatically but opaquely: on a fresh Session it sometimes reads its engine
  context from the cache and sometimes writes all of it again, so the same call
  can cost several times more from one fresh Session to the next.

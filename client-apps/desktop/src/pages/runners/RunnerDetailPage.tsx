...
export default function RunnerDetailPage() {
  ...
  const handleStop = useCallback(async () => {
    ...
    const confirmed = await confirm({
      ...
      description:
        "The runner will stop accepting new executions. Running executions will be allowed to complete.",
      confirmLabel: "Stop",
      variant: "destructive",
    });
    if (confirmed) {
      try {
        await stop({ runnerId: id, reason: "stopped via desktop console" });
  ...
  const handleDelete = useCallback(async () => {
    const confirmed = await confirm({
      ...
      description:
        "This action cannot be undone. The runner will be permanently removed.",
      confirmLabel: "Delete",
      variant: "destructive",
    });
    if (confirmed) {
  ...
  }, [confirm, deleteResource, navigate, runnerName]);

...
// program.Println (direct p.msgs enqueue) then sends streamingShowMsg to
// activate progressive mode. Committing the header renderer-side avoids
// a race where tea.Println Cmds execute in goroutines that can be
// overtaken by subsequent streamingUpdateMsg messages, causing tool
// content to appear above the header.
//
...
// commitStreamHeader commits a pre-approval streaming header directly
// to Bubbletea's scrollback via program.Println. This enqueues the
// printLineMessage on the same p.msgs channel as subsequent Send calls,
// guaranteeing FIFO ordering and preventing tool content from appearing
// before the header.

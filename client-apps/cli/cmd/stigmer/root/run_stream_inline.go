...
func (r *inlineRenderer) handleEvent(ctx context.Context, event executiontui.Event) (done bool, phase string, exitErr string) {
	...
	// Initiate pre-approval streaming for any tool whose input is being
	// generated by the AI (streaming_source == INPUT). The content is
	// committed to scrollback progressively as lines complete, giving
	// the user a live typewriter view of what the agent is producing.
	// ToolWaitingApprovalEvent transitions to the approval flow.
	// Read, think, and task tools are already intercepted above this point.
	//
	// Falls back to IsStreaming when streaming_source is not yet populated
	// (backward compat with older backend versions).
	if e, ok := event.(executiontui.ToolRunningEvent); ok && isInputStreaming(e.ToolCall) {
		r.flushPendingReads()

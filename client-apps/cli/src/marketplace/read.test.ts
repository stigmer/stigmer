...
// official tree in this repository reading clean with `assistant` offered.
...
describe("the official tree in this repository", () => {
  it("reads clean through the CLI walker and offers assistant as its one default", () => {
    ...
    expect(tree.marketplace.defaults).toEqual(["assistant"]);
    expect(findEntry(tree.marketplace, "assistant")?.dir).toBe("assistant");
  });
});

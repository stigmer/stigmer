...
export const GET_BINDINGS: ReadonlyMap<ApiResourceKind, Getter> = new Map([
  ...
  [
    ...
    refGetter(ApiResourceKind.schedule, ScheduleSchema, (c) => c.schedule),
  ],
  [
    ApiResourceKind.skill,
    refGetter(ApiResourceKind.skill, SkillSchema, (c) => c.skill),
  ],
  [
    ApiResourceKind.plugin,
    refGetter(ApiResourceKind.plugin, PluginSchema, (c) => c.plugin),
  ],
  [
...
function refGetter(
  ...
  schema: DescMessage,
  pick: (c: Stigmer) => RefClient,
): Getter {
  return async (client, ref) => {
    const sub = pick(client);
    const message =
      ref.kind === "id"
        ? await sub.get(ref.id)
        : await sub.getByReference({ org: ref.org, slug: ref.slug, kind });
    return { schema, message };
  };
}

// A resource fetchable only by ID (e.g. API keys are not slug-addressable).
function idOnlyGetter(
  schema: DescMessage,
  pick: (c: Stigmer) => { get(id: string): Promise<Message> },
  label: string,
): Getter {
  return async (client, ref) => {
    if (ref.kind !== "id") {

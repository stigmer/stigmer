...
describe("resolveSeedpackContent", () => {
  ...
  it("excludes the non-resource canary manifest from the content set", () => {
    // The CI canary manifest has no `kind`; if it were staged it would break the
    // declarative-apply bootstrap. It lives in canary/ (outside SEEDPACK_ENTRIES).
    const files = listContentFiles(resolveSeedpackContent().dir);

/**
 * Authentication flow playback scenario for the federation guide.
 *
 * Walks through the end-to-end flow when a federated user calls the
 * Stigmer API: platform login → JWT issued → API call → token
 * validation → identity resolution → authorization → response,
 * plus error scenarios for 401 and 403.
 *
 * Uses three view types (BrowserView, TerminalView, APIExchangeView)
 * to visually distinguish the platform, the network, and the Stigmer
 * API internals.
 */

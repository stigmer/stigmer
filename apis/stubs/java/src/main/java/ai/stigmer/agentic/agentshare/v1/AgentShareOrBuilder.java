// Generated by the protocol buffer compiler.  DO NOT EDIT!
// NO CHECKED-IN PROTOBUF GENCODE
// source: ai/stigmer/agentic/agentshare/v1/api.proto
// Protobuf Java Version: 4.34.0

package ai.stigmer.agentic.agentshare.v1;

@com.google.protobuf.Generated
public interface AgentShareOrBuilder extends
    // @@protoc_insertion_point(interface_extends:ai.stigmer.agentic.agentshare.v1.AgentShare)
    com.google.protobuf.MessageOrBuilder {

  /**
   * <pre>
   * API version for this resource type.
   * </pre>
   *
   * <code>string api_version = 1 [json_name = "apiVersion", (.buf.validate.field) = { ... }</code>
   * @return The apiVersion.
   */
  java.lang.String getApiVersion();
  /**
   * <pre>
   * API version for this resource type.
   * </pre>
   *
   * <code>string api_version = 1 [json_name = "apiVersion", (.buf.validate.field) = { ... }</code>
   * @return The bytes for apiVersion.
   */
  com.google.protobuf.ByteString
      getApiVersionBytes();

  /**
   * <pre>
   * Resource kind identifier.
   * </pre>
   *
   * <code>string kind = 2 [json_name = "kind", (.buf.validate.field) = { ... }</code>
   * @return The kind.
   */
  java.lang.String getKind();
  /**
   * <pre>
   * Resource kind identifier.
   * </pre>
   *
   * <code>string kind = 2 [json_name = "kind", (.buf.validate.field) = { ... }</code>
   * @return The bytes for kind.
   */
  com.google.protobuf.ByteString
      getKindBytes();

  /**
   * <pre>
   * Resource metadata including name, organization, and labels.
   *
   * The org and slug form the share's hosted chat URL
   * (`/chat/&lt;org&gt;/&lt;slug&gt;`). The slug defaults to the referenced agent's
   * slug and is immutable once created — to change a link, create a new
   * share and delete the old one.
   * </pre>
   *
   * <code>.ai.stigmer.commons.apiresource.ApiResourceMetadata metadata = 3 [json_name = "metadata", (.buf.validate.field) = { ... }</code>
   * @return Whether the metadata field is set.
   */
  boolean hasMetadata();
  /**
   * <pre>
   * Resource metadata including name, organization, and labels.
   *
   * The org and slug form the share's hosted chat URL
   * (`/chat/&lt;org&gt;/&lt;slug&gt;`). The slug defaults to the referenced agent's
   * slug and is immutable once created — to change a link, create a new
   * share and delete the old one.
   * </pre>
   *
   * <code>.ai.stigmer.commons.apiresource.ApiResourceMetadata metadata = 3 [json_name = "metadata", (.buf.validate.field) = { ... }</code>
   * @return The metadata.
   */
  ai.stigmer.commons.apiresource.ApiResourceMetadata getMetadata();
  /**
   * <pre>
   * Resource metadata including name, organization, and labels.
   *
   * The org and slug form the share's hosted chat URL
   * (`/chat/&lt;org&gt;/&lt;slug&gt;`). The slug defaults to the referenced agent's
   * slug and is immutable once created — to change a link, create a new
   * share and delete the old one.
   * </pre>
   *
   * <code>.ai.stigmer.commons.apiresource.ApiResourceMetadata metadata = 3 [json_name = "metadata", (.buf.validate.field) = { ... }</code>
   */
  ai.stigmer.commons.apiresource.ApiResourceMetadataOrBuilder getMetadataOrBuilder();

  /**
   * <pre>
   * Configurable properties: referenced agent, audience, origins, messages, and credentials.
   * </pre>
   *
   * <code>.ai.stigmer.agentic.agentshare.v1.AgentShareSpec spec = 4 [json_name = "spec"];</code>
   * @return Whether the spec field is set.
   */
  boolean hasSpec();
  /**
   * <pre>
   * Configurable properties: referenced agent, audience, origins, messages, and credentials.
   * </pre>
   *
   * <code>.ai.stigmer.agentic.agentshare.v1.AgentShareSpec spec = 4 [json_name = "spec"];</code>
   * @return The spec.
   */
  ai.stigmer.agentic.agentshare.v1.AgentShareSpec getSpec();
  /**
   * <pre>
   * Configurable properties: referenced agent, audience, origins, messages, and credentials.
   * </pre>
   *
   * <code>.ai.stigmer.agentic.agentshare.v1.AgentShareSpec spec = 4 [json_name = "spec"];</code>
   */
  ai.stigmer.agentic.agentshare.v1.AgentShareSpecOrBuilder getSpecOrBuilder();

  /**
   * <pre>
   * System-managed state including audit trail and the rotatable link token.
   * </pre>
   *
   * <code>.ai.stigmer.agentic.agentshare.v1.AgentShareStatus status = 5 [json_name = "status"];</code>
   * @return Whether the status field is set.
   */
  boolean hasStatus();
  /**
   * <pre>
   * System-managed state including audit trail and the rotatable link token.
   * </pre>
   *
   * <code>.ai.stigmer.agentic.agentshare.v1.AgentShareStatus status = 5 [json_name = "status"];</code>
   * @return The status.
   */
  ai.stigmer.agentic.agentshare.v1.AgentShareStatus getStatus();
  /**
   * <pre>
   * System-managed state including audit trail and the rotatable link token.
   * </pre>
   *
   * <code>.ai.stigmer.agentic.agentshare.v1.AgentShareStatus status = 5 [json_name = "status"];</code>
   */
  ai.stigmer.agentic.agentshare.v1.AgentShareStatusOrBuilder getStatusOrBuilder();
}

// Generated by the protocol buffer compiler.  DO NOT EDIT!
// NO CHECKED-IN PROTOBUF GENCODE
// source: ai/stigmer/billing/plan/v1/api.proto
// Protobuf Java Version: 4.34.0

package ai.stigmer.billing.plan.v1;

@com.google.protobuf.Generated
public interface PlanOrBuilder extends
    // @@protoc_insertion_point(interface_extends:ai.stigmer.billing.plan.v1.Plan)
    com.google.protobuf.MessageOrBuilder {

  /**
   * <pre>
   * API version for this resource type.
   * </pre>
   *
   * <code>string api_version = 1 [json_name = "apiVersion", (.buf.validate.field) = { ... }</code>
   * @return The apiVersion.
   */
  java.lang.String getApiVersion();
  /**
   * <pre>
   * API version for this resource type.
   * </pre>
   *
   * <code>string api_version = 1 [json_name = "apiVersion", (.buf.validate.field) = { ... }</code>
   * @return The bytes for apiVersion.
   */
  com.google.protobuf.ByteString
      getApiVersionBytes();

  /**
   * <pre>
   * Resource kind identifier.
   * </pre>
   *
   * <code>string kind = 2 [json_name = "kind", (.buf.validate.field) = { ... }</code>
   * @return The kind.
   */
  java.lang.String getKind();
  /**
   * <pre>
   * Resource kind identifier.
   * </pre>
   *
   * <code>string kind = 2 [json_name = "kind", (.buf.validate.field) = { ... }</code>
   * @return The bytes for kind.
   */
  com.google.protobuf.ByteString
      getKindBytes();

  /**
   * <pre>
   * Standard resource metadata. A plan has no organization: metadata.org is
   * empty, and the slug is the plan's stable handle ("team", "business").
   * </pre>
   *
   * <code>.ai.stigmer.commons.apiresource.ApiResourceMetadata metadata = 3 [json_name = "metadata", (.buf.validate.field) = { ... }</code>
   * @return Whether the metadata field is set.
   */
  boolean hasMetadata();
  /**
   * <pre>
   * Standard resource metadata. A plan has no organization: metadata.org is
   * empty, and the slug is the plan's stable handle ("team", "business").
   * </pre>
   *
   * <code>.ai.stigmer.commons.apiresource.ApiResourceMetadata metadata = 3 [json_name = "metadata", (.buf.validate.field) = { ... }</code>
   * @return The metadata.
   */
  ai.stigmer.commons.apiresource.ApiResourceMetadata getMetadata();
  /**
   * <pre>
   * Standard resource metadata. A plan has no organization: metadata.org is
   * empty, and the slug is the plan's stable handle ("team", "business").
   * </pre>
   *
   * <code>.ai.stigmer.commons.apiresource.ApiResourceMetadata metadata = 3 [json_name = "metadata", (.buf.validate.field) = { ... }</code>
   */
  ai.stigmer.commons.apiresource.ApiResourceMetadataOrBuilder getMetadataOrBuilder();

  /**
   * <pre>
   * Operator-provided plan definition: the instrument, the entitlements and
   * the terms.
   * </pre>
   *
   * <code>.ai.stigmer.billing.plan.v1.PlanSpec spec = 4 [json_name = "spec"];</code>
   * @return Whether the spec field is set.
   */
  boolean hasSpec();
  /**
   * <pre>
   * Operator-provided plan definition: the instrument, the entitlements and
   * the terms.
   * </pre>
   *
   * <code>.ai.stigmer.billing.plan.v1.PlanSpec spec = 4 [json_name = "spec"];</code>
   * @return The spec.
   */
  ai.stigmer.billing.plan.v1.PlanSpec getSpec();
  /**
   * <pre>
   * Operator-provided plan definition: the instrument, the entitlements and
   * the terms.
   * </pre>
   *
   * <code>.ai.stigmer.billing.plan.v1.PlanSpec spec = 4 [json_name = "spec"];</code>
   */
  ai.stigmer.billing.plan.v1.PlanSpecOrBuilder getSpecOrBuilder();

  /**
   * <pre>
   * System-managed state: audit and lifecycle.
   * </pre>
   *
   * <code>.ai.stigmer.billing.plan.v1.PlanStatus status = 5 [json_name = "status"];</code>
   * @return Whether the status field is set.
   */
  boolean hasStatus();
  /**
   * <pre>
   * System-managed state: audit and lifecycle.
   * </pre>
   *
   * <code>.ai.stigmer.billing.plan.v1.PlanStatus status = 5 [json_name = "status"];</code>
   * @return The status.
   */
  ai.stigmer.billing.plan.v1.PlanStatus getStatus();
  /**
   * <pre>
   * System-managed state: audit and lifecycle.
   * </pre>
   *
   * <code>.ai.stigmer.billing.plan.v1.PlanStatus status = 5 [json_name = "status"];</code>
   */
  ai.stigmer.billing.plan.v1.PlanStatusOrBuilder getStatusOrBuilder();
}

// Generated by the protocol buffer compiler.  DO NOT EDIT!
// NO CHECKED-IN PROTOBUF GENCODE
// source: ai/stigmer/agentic/agent/v1/api.proto
// Protobuf Java Version: 4.34.0

package ai.stigmer.agentic.agent.v1;

@com.google.protobuf.Generated
public interface AgentOrBuilder extends
    // @@protoc_insertion_point(interface_extends:ai.stigmer.agentic.agent.v1.Agent)
    com.google.protobuf.MessageOrBuilder {

  /**
   * <pre>
   * API version for this resource type.
   * </pre>
   *
   * <code>string api_version = 1 [json_name = "apiVersion", (.buf.validate.field) = { ... }</code>
   * @return The apiVersion.
   */
  java.lang.String getApiVersion();
  /**
   * <pre>
   * API version for this resource type.
   * </pre>
   *
   * <code>string api_version = 1 [json_name = "apiVersion", (.buf.validate.field) = { ... }</code>
   * @return The bytes for apiVersion.
   */
  com.google.protobuf.ByteString
      getApiVersionBytes();

  /**
   * <pre>
   * Resource kind identifier.
   * </pre>
   *
   * <code>string kind = 2 [json_name = "kind", (.buf.validate.field) = { ... }</code>
   * @return The kind.
   */
  java.lang.String getKind();
  /**
   * <pre>
   * Resource kind identifier.
   * </pre>
   *
   * <code>string kind = 2 [json_name = "kind", (.buf.validate.field) = { ... }</code>
   * @return The bytes for kind.
   */
  com.google.protobuf.ByteString
      getKindBytes();

  /**
   * <pre>
   * Standard resource metadata including name, id, labels, and tags.
   * Agents belong to an organization and can be PUBLIC or PRIVATE.
   * Reference format: "org/slug" (e.g., "stigmer/web-search", "acme/data-analyst")
   * </pre>
   *
   * <code>.ai.stigmer.commons.apiresource.ApiResourceMetadata metadata = 3 [json_name = "metadata", (.buf.validate.field) = { ... }</code>
   * @return Whether the metadata field is set.
   */
  boolean hasMetadata();
  /**
   * <pre>
   * Standard resource metadata including name, id, labels, and tags.
   * Agents belong to an organization and can be PUBLIC or PRIVATE.
   * Reference format: "org/slug" (e.g., "stigmer/web-search", "acme/data-analyst")
   * </pre>
   *
   * <code>.ai.stigmer.commons.apiresource.ApiResourceMetadata metadata = 3 [json_name = "metadata", (.buf.validate.field) = { ... }</code>
   * @return The metadata.
   */
  ai.stigmer.commons.apiresource.ApiResourceMetadata getMetadata();
  /**
   * <pre>
   * Standard resource metadata including name, id, labels, and tags.
   * Agents belong to an organization and can be PUBLIC or PRIVATE.
   * Reference format: "org/slug" (e.g., "stigmer/web-search", "acme/data-analyst")
   * </pre>
   *
   * <code>.ai.stigmer.commons.apiresource.ApiResourceMetadata metadata = 3 [json_name = "metadata", (.buf.validate.field) = { ... }</code>
   */
  ai.stigmer.commons.apiresource.ApiResourceMetadataOrBuilder getMetadataOrBuilder();

  /**
   * <pre>
   * Agent-specific configuration.
   * </pre>
   *
   * <code>.ai.stigmer.agentic.agent.v1.AgentSpec spec = 4 [json_name = "spec"];</code>
   * @return Whether the spec field is set.
   */
  boolean hasSpec();
  /**
   * <pre>
   * Agent-specific configuration.
   * </pre>
   *
   * <code>.ai.stigmer.agentic.agent.v1.AgentSpec spec = 4 [json_name = "spec"];</code>
   * @return The spec.
   */
  ai.stigmer.agentic.agent.v1.AgentSpec getSpec();
  /**
   * <pre>
   * Agent-specific configuration.
   * </pre>
   *
   * <code>.ai.stigmer.agentic.agent.v1.AgentSpec spec = 4 [json_name = "spec"];</code>
   */
  ai.stigmer.agentic.agent.v1.AgentSpecOrBuilder getSpecOrBuilder();

  /**
   * <pre>
   * System-managed status containing audit information and default instance ID.
   * </pre>
   *
   * <code>.ai.stigmer.agentic.agent.v1.AgentStatus status = 5 [json_name = "status"];</code>
   * @return Whether the status field is set.
   */
  boolean hasStatus();
  /**
   * <pre>
   * System-managed status containing audit information and default instance ID.
   * </pre>
   *
   * <code>.ai.stigmer.agentic.agent.v1.AgentStatus status = 5 [json_name = "status"];</code>
   * @return The status.
   */
  ai.stigmer.agentic.agent.v1.AgentStatus getStatus();
  /**
   * <pre>
   * System-managed status containing audit information and default instance ID.
   * </pre>
   *
   * <code>.ai.stigmer.agentic.agent.v1.AgentStatus status = 5 [json_name = "status"];</code>
   */
  ai.stigmer.agentic.agent.v1.AgentStatusOrBuilder getStatusOrBuilder();
}

// Generated by the protocol buffer compiler.  DO NOT EDIT!
// NO CHECKED-IN PROTOBUF GENCODE
// source: ai/stigmer/agentic/agentrunner/v1/io.proto
// Protobuf Java Version: 4.34.0

package ai.stigmer.agentic.agentrunner.v1;

@com.google.protobuf.Generated
public interface ListAgentRunnersRequestOrBuilder extends
    // @@protoc_insertion_point(interface_extends:ai.stigmer.agentic.agentrunner.v1.ListAgentRunnersRequest)
    com.google.protobuf.MessageOrBuilder {

  /**
   * <pre>
   * Organization slug to scope the listing.
   * </pre>
   *
   * <code>string org = 1 [json_name = "org", (.buf.validate.field) = { ... }</code>
   * @return The org.
   */
  java.lang.String getOrg();
  /**
   * <pre>
   * Organization slug to scope the listing.
   * </pre>
   *
   * <code>string org = 1 [json_name = "org", (.buf.validate.field) = { ... }</code>
   * @return The bytes for org.
   */
  com.google.protobuf.ByteString
      getOrgBytes();

  /**
   * <pre>
   * Filter by metadata labels. AND semantics: all labels must match.
   *
   * Common filters:
   * - Exclude system-managed: omit {"stigmer.ai/system-managed": "true"}
   * - Show only system-managed: {"stigmer.ai/system-managed": "true"}
   * </pre>
   *
   * <code>map&lt;string, string&gt; labels = 2 [json_name = "labels"];</code>
   */
  int getLabelsCount();
  /**
   * <pre>
   * Filter by metadata labels. AND semantics: all labels must match.
   *
   * Common filters:
   * - Exclude system-managed: omit {"stigmer.ai/system-managed": "true"}
   * - Show only system-managed: {"stigmer.ai/system-managed": "true"}
   * </pre>
   *
   * <code>map&lt;string, string&gt; labels = 2 [json_name = "labels"];</code>
   */
  boolean containsLabels(
      java.lang.String key);
  /**
   * Use {@link #getLabelsMap()} instead.
   */
  @java.lang.Deprecated
  java.util.Map<java.lang.String, java.lang.String>
  getLabels();
  /**
   * <pre>
   * Filter by metadata labels. AND semantics: all labels must match.
   *
   * Common filters:
   * - Exclude system-managed: omit {"stigmer.ai/system-managed": "true"}
   * - Show only system-managed: {"stigmer.ai/system-managed": "true"}
   * </pre>
   *
   * <code>map&lt;string, string&gt; labels = 2 [json_name = "labels"];</code>
   */
  java.util.Map<java.lang.String, java.lang.String>
  getLabelsMap();
  /**
   * <pre>
   * Filter by metadata labels. AND semantics: all labels must match.
   *
   * Common filters:
   * - Exclude system-managed: omit {"stigmer.ai/system-managed": "true"}
   * - Show only system-managed: {"stigmer.ai/system-managed": "true"}
   * </pre>
   *
   * <code>map&lt;string, string&gt; labels = 2 [json_name = "labels"];</code>
   */
  /* nullable */
java.lang.String getLabelsOrDefault(
      java.lang.String key,
      /* nullable */
java.lang.String defaultValue);
  /**
   * <pre>
   * Filter by metadata labels. AND semantics: all labels must match.
   *
   * Common filters:
   * - Exclude system-managed: omit {"stigmer.ai/system-managed": "true"}
   * - Show only system-managed: {"stigmer.ai/system-managed": "true"}
   * </pre>
   *
   * <code>map&lt;string, string&gt; labels = 2 [json_name = "labels"];</code>
   */
  java.lang.String getLabelsOrThrow(
      java.lang.String key);

  /**
   * <pre>
   * Pagination options.
   * </pre>
   *
   * <code>.ai.stigmer.commons.rpc.PageInfo page_info = 3 [json_name = "pageInfo"];</code>
   * @return Whether the pageInfo field is set.
   */
  boolean hasPageInfo();
  /**
   * <pre>
   * Pagination options.
   * </pre>
   *
   * <code>.ai.stigmer.commons.rpc.PageInfo page_info = 3 [json_name = "pageInfo"];</code>
   * @return The pageInfo.
   */
  ai.stigmer.commons.rpc.PageInfo getPageInfo();
  /**
   * <pre>
   * Pagination options.
   * </pre>
   *
   * <code>.ai.stigmer.commons.rpc.PageInfo page_info = 3 [json_name = "pageInfo"];</code>
   */
  ai.stigmer.commons.rpc.PageInfoOrBuilder getPageInfoOrBuilder();
}

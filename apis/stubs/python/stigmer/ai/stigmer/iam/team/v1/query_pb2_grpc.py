# Generated by the gRPC Python protocol compiler plugin. DO NOT EDIT!
"""Client and server classes corresponding to protobuf-defined services."""
import grpc

from ai.stigmer.commons.apiresource import io_pb2 as ai_dot_stigmer_dot_commons_dot_apiresource_dot_io__pb2
from ai.stigmer.iam.team.v1 import api_pb2 as ai_dot_stigmer_dot_iam_dot_team_dot_v1_dot_api__pb2
from ai.stigmer.iam.team.v1 import io_pb2 as ai_dot_stigmer_dot_iam_dot_team_dot_v1_dot_io__pb2


class TeamQueryControllerStub(object):
    """TeamQueryController provides read operations for teams.

    Every member of an organization can see its teams, so anyone who can share
    a resource can pick a team to share it with. Who is in a team is read
    through the IAM policy service's access list on the team.
    """

    def __init__(self, channel):
        """Constructor.

        Args:
            channel: A grpc.Channel.
        """
        self.get = channel.unary_unary(
                '/ai.stigmer.iam.team.v1.TeamQueryController/get',
                request_serializer=ai_dot_stigmer_dot_commons_dot_apiresource_dot_io__pb2.ApiResourceId.SerializeToString,
                response_deserializer=ai_dot_stigmer_dot_iam_dot_team_dot_v1_dot_api__pb2.Team.FromString,
                _registered_method=True)
        self.getByReference = channel.unary_unary(
                '/ai.stigmer.iam.team.v1.TeamQueryController/getByReference',
                request_serializer=ai_dot_stigmer_dot_commons_dot_apiresource_dot_io__pb2.ApiResourceReference.SerializeToString,
                response_deserializer=ai_dot_stigmer_dot_iam_dot_team_dot_v1_dot_api__pb2.Team.FromString,
                _registered_method=True)
        self.listByOrg = channel.unary_unary(
                '/ai.stigmer.iam.team.v1.TeamQueryController/listByOrg',
                request_serializer=ai_dot_stigmer_dot_iam_dot_team_dot_v1_dot_io__pb2.ListTeamsByOrgInput.SerializeToString,
                response_deserializer=ai_dot_stigmer_dot_iam_dot_team_dot_v1_dot_io__pb2.Teams.FromString,
                _registered_method=True)


class TeamQueryControllerServicer(object):
    """TeamQueryController provides read operations for teams.

    Every member of an organization can see its teams, so anyone who can share
    a resource can pick a team to share it with. Who is in a team is read
    through the IAM policy service's access list on the team.
    """

    def get(self, request, context):
        """Get a team by its unique identifier.
        """
        context.set_code(grpc.StatusCode.UNIMPLEMENTED)
        context.set_details('Method not implemented!')
        raise NotImplementedError('Method not implemented!')

    def getByReference(self, request, context):
        """Get a team by its organization-scoped reference (org/slug).
        """
        context.set_code(grpc.StatusCode.UNIMPLEMENTED)
        context.set_details('Method not implemented!')
        raise NotImplementedError('Method not implemented!')

    def listByOrg(self, request, context):
        """List the teams of an organization.
        """
        context.set_code(grpc.StatusCode.UNIMPLEMENTED)
        context.set_details('Method not implemented!')
        raise NotImplementedError('Method not implemented!')


def add_TeamQueryControllerServicer_to_server(servicer, server):
    rpc_method_handlers = {
            'get': grpc.unary_unary_rpc_method_handler(
                    servicer.get,
                    request_deserializer=ai_dot_stigmer_dot_commons_dot_apiresource_dot_io__pb2.ApiResourceId.FromString,
                    response_serializer=ai_dot_stigmer_dot_iam_dot_team_dot_v1_dot_api__pb2.Team.SerializeToString,
            ),
            'getByReference': grpc.unary_unary_rpc_method_handler(
                    servicer.getByReference,
                    request_deserializer=ai_dot_stigmer_dot_commons_dot_apiresource_dot_io__pb2.ApiResourceReference.FromString,
                    response_serializer=ai_dot_stigmer_dot_iam_dot_team_dot_v1_dot_api__pb2.Team.SerializeToString,
            ),
            'listByOrg': grpc.unary_unary_rpc_method_handler(
                    servicer.listByOrg,
                    request_deserializer=ai_dot_stigmer_dot_iam_dot_team_dot_v1_dot_io__pb2.ListTeamsByOrgInput.FromString,
                    response_serializer=ai_dot_stigmer_dot_iam_dot_team_dot_v1_dot_io__pb2.Teams.SerializeToString,
            ),
    }
    generic_handler = grpc.method_handlers_generic_handler(
            'ai.stigmer.iam.team.v1.TeamQueryController', rpc_method_handlers)
    server.add_generic_rpc_handlers((generic_handler,))
    server.add_registered_method_handlers('ai.stigmer.iam.team.v1.TeamQueryController', rpc_method_handlers)


 # This class is part of an EXPERIMENTAL API.
class TeamQueryController(object):
    """TeamQueryController provides read operations for teams.

    Every member of an organization can see its teams, so anyone who can share
    a resource can pick a team to share it with. Who is in a team is read
    through the IAM policy service's access list on the team.
    """

    @staticmethod
    def get(request,
            target,
            options=(),
            channel_credentials=None,
            call_credentials=None,
            insecure=False,
            compression=None,
            wait_for_ready=None,
            timeout=None,
            metadata=None):
        return grpc.experimental.unary_unary(
            request,
            target,
            '/ai.stigmer.iam.team.v1.TeamQueryController/get',
            ai_dot_stigmer_dot_commons_dot_apiresource_dot_io__pb2.ApiResourceId.SerializeToString,
            ai_dot_stigmer_dot_iam_dot_team_dot_v1_dot_api__pb2.Team.FromString,
            options,
            channel_credentials,
            insecure,
            call_credentials,
            compression,
            wait_for_ready,
            timeout,
            metadata,
            _registered_method=True)

    @staticmethod
    def getByReference(request,
            target,
            options=(),
            channel_credentials=None,
            call_credentials=None,
            insecure=False,
            compression=None,
            wait_for_ready=None,
            timeout=None,
            metadata=None):
        return grpc.experimental.unary_unary(
            request,
            target,
            '/ai.stigmer.iam.team.v1.TeamQueryController/getByReference',
            ai_dot_stigmer_dot_commons_dot_apiresource_dot_io__pb2.ApiResourceReference.SerializeToString,
            ai_dot_stigmer_dot_iam_dot_team_dot_v1_dot_api__pb2.Team.FromString,
            options,
            channel_credentials,
            insecure,
            call_credentials,
            compression,
            wait_for_ready,
            timeout,
            metadata,
            _registered_method=True)

    @staticmethod
    def listByOrg(request,
            target,
            options=(),
            channel_credentials=None,
            call_credentials=None,
            insecure=False,
            compression=None,
            wait_for_ready=None,
            timeout=None,
            metadata=None):
        return grpc.experimental.unary_unary(
            request,
            target,
            '/ai.stigmer.iam.team.v1.TeamQueryController/listByOrg',
            ai_dot_stigmer_dot_iam_dot_team_dot_v1_dot_io__pb2.ListTeamsByOrgInput.SerializeToString,
            ai_dot_stigmer_dot_iam_dot_team_dot_v1_dot_io__pb2.Teams.FromString,
            options,
            channel_credentials,
            insecure,
            call_credentials,
            compression,
            wait_for_ready,
            timeout,
            metadata,
            _registered_method=True)

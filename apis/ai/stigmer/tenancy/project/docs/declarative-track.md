...
The CLI recursively scans the directory rooted at `stigmer.yaml` for YAML files. Any file containing a valid Stigmer API resource (with a recognized `apiVersion` such as `agentic.stigmer.ai/v1` or `tenancy.stigmer.ai/v1`, and a supported `kind`) is discovered and applied. Subdirectories are scanned automatically — the layout above is a convention, not a requirement.
...
| **Valid resource required** | Files must contain a recognized Stigmer `apiVersion` (e.g., `agentic.stigmer.ai/v1`, `tenancy.stigmer.ai/v1`) and a supported `kind`. Non-resource YAML files (CI configs, Kubernetes manifests, etc.) are skipped. |

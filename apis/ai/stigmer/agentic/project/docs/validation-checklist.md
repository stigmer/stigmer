...
- [ ] All resource files alongside `stigmer.yaml` contain `apiVersion: agentic.stigmer.ai/v1`
- [ ] All resource files have a valid `kind` (`Agent`, `McpServer`, `Skill`, etc.)
- [ ] Non-Stigmer YAML files (CI configs, Kubernetes manifests) do not contain `apiVersion: agentic.stigmer.ai/v1` — they will be ignored automatically, but verify none are misidentified
...
If a non-Stigmer YAML file in the project directory contains `apiVersion: agentic.stigmer.ai/v1`, the CLI will attempt to apply it as a Stigmer resource. Keep non-Stigmer YAML files (Kubernetes manifests, CI configs) out of the project directory, or in a subdirectory named `.nostigmer` (excluded from scan).

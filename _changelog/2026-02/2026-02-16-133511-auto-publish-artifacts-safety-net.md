...
Implemented a structural safety net that automatically publishes files written by agents as downloadable artifacts when the agent completes execution without calling `publish_artifact`. This eliminates dependency on LLM behavioral compliance for delivering agent output to users, ensuring 100% artifact delivery reliability regardless of LLM decision-making.
...
Replace LLM-dependent artifact publishing with a **post-stream safety net** that runs after the agent execution completes. The platform now automatically detects completed `write` / `write_file` tool calls and publishes the written files as downloadable artifacts, preserving the original folder structure.
...
Added `_auto_publish_written_files()` async function (150 lines) that:
- Scans completed tool calls for `write` / `write_file` operations
...
2. Zero artifacts were published by the agent (`status_builder._artifacts` is empty)
3. At least one `write`/`write_file` tool call completed successfully
...
**All tests passing**: 12/12 new tests, 168/172 existing tests (4 pre-existing failures unrelated to changes).

## Benefits

### User Experience
- **100% artifact delivery**: Users always receive downloadable output when agents write files, regardless of LLM behavior
...
- All future agents that write files
...
backend/services/agent-runner/worker/activities/execute_graphton.py
  +150 lines: _auto_publish_written_files() helper function
  +30 lines: Post-stream auto-publish wiring
...
backend/services/agent-runner/tests/test_auto_publish.py
  +373 lines: 12 comprehensive unit tests (new file)
```

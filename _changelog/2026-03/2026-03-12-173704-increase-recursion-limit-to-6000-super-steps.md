...
to effectively unlimited (10,000,000 super-steps). Loop detection middleware
is now the sole behavioral safety mechanism. The `ExecutionBudgetMiddleware`
is only injected when an explicit limit is set via `max_tool_rounds`. This
eliminates premature agent termination from `GraphRecursionError` while
reducing per-round super-step cost.
...
Make `recursion_limit=None` the default. Internally, `None` maps to
10,000,000 super-steps — an explicit override is required because
deepagents internally sets its own lower recursion_limit, and skipping
`with_config` would inherit that ceiling. Loop detection middleware
(7 consecutive / 20 total duplicate patterns) catches stuck agents
regardless of super-step count. Per-execution limits remain available
via `max_tool_rounds` in `ExecutionConfig` proto.
...
|----------|--------|-------|
| Default execution | 6000 super-step ceiling | 10M super-steps (effectively unlimited) |
...
**Source code:**
- `graphton/core/agent.py` — `recursion_limit: int | None = None`; maps to
  10M internally via `with_config`; skip `ExecutionBudgetMiddleware` when None
...
- `test_recursion_limit.py` — New tests for None default: `test_default_is_none`,
  `test_default_none_uses_unlimited` (verifies 10M), removed warning threshold tests
...
- **All agent executions**: Run until task completion or loop detection (10M ceiling)
...
- Follows from Session 7 (100→1000), Session 8 (1000→6000), and Session 9 (unlimited) fixes
- Session 10: discovered deepagents sets its own recursion_limit; must explicitly override

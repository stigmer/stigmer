...
type WorkflowInstanceInput struct {
	...
	WorkflowId string `json:"workflow_id,omitempty" jsonschema:"description=Reference to the Workflow template this instance deploys. This links the instance to a reusable orchestration blueprint. The Workflow defines which AgentInstances to orchestrate and in what order. Format: Workflow resource ID (e.g., 'wfl-abc123') Validation: Minimum length of 1 character (required field) Example: 'wfl-abc123' (references a Workflow named 'deploy-to-cloud')"`
	...
	EnvRefs []ApiResourceReferenceInput `json:"env_refs,omitempty" jsonschema:"description=References to Environment resources providing configuration and secrets. Environments are layered configuration containers that provide: - Environment variables (API keys, endpoints, flags) - Secrets (credentials, tokens, passwords) - Configuration values (timeouts, limits, settings) Layering Behavior: Environments are merged in order - later environments override earlier ones. This enables a base + overrides pattern: Example layering: [base-env, aws-prod-env, github-team-env] └─ base-env: Common settings for all instances └─ aws-prod-env: AWS production credentials (overrides base AWS settings) └─ github-team-env: Team-specific GitHub tokens (overrides generic tokens) Use Cases: - Single env: [prod-env] - Simple, all config in one place - Base + specific: [base, prod] - Common config + environment-specific - Layered: [base, cloud, team] - Base + cloud credentials + team settings References use ApiResourceReference which supports: - By ID: {id: 'env-abc123'} - By slug: {slug: 'aws-prod-env'} At execution time, the WorkflowExecution runtime merges these environments and provides the combined configuration to all agents in the workflow."`
}
...
func (input *WorkflowInstanceInput) specToProto() (*workflowinstancev1.WorkflowInstanceSpec, error) {
	...
	for _, item := range input.EnvRefs {
		v, err := item.toProto()
		if err != nil {
			return nil, err
		}
		spec.EnvRefs = append(spec.EnvRefs, v)
	}
	return spec, nil
}

...
type WorkflowInstanceInput struct {
	...
	// Who can observe the run history (executions) of this instance. Independent of the instance's own visibility: making an instance org-visible lets teammates see and run it, but does NOT expose each other's run inputs/outputs unless this is set to ORGANIZATION. Defaults to PRIVATE (unspecified is treated as private) — each execution is visible only to the user who triggered it. Update via WorkflowInstanceCommandController.updateExecutionVisibility.
	ExecutionVisibility string `json:"execution_visibility,omitempty" jsonschema:"Who can observe the run history (executions) of this instance. Independent of the instance's own visibility: making an instance org-visible lets teammates see and run it, but does NOT expose each other's run inputs/outputs unless this is set to ORGANIZATION. Defaults to PRIVATE (unspecified is treated as private) — each execution is visible only to the user who triggered it. Update via WorkflowInstanceCommandController.updateExecutionVisibility. Allowed values: workflow_execution_visibility_private, workflow_execution_visibility_organization."`
}
...
func (input *WorkflowInstanceInput) specToProto() (*workflowinstancev1.WorkflowInstanceSpec, error) {
	...
	spec.ExecutionVisibility = workflowinstancev1.WorkflowExecutionVisibility(workflowinstancev1.WorkflowExecutionVisibility_value[input.ExecutionVisibility])
	return spec, nil
}

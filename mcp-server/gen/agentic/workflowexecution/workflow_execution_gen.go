...
type WorkflowExecutionInput struct {
	...
	TriggerMetadata map[string]string `json:"trigger_metadata,omitempty" jsonschema:"Trigger context metadata. Contains contextual information about who/what triggered this execution and how. This metadata is NOT used by the workflow logic itself - it's for audit, debugging, and analytics. Common metadata keys: - 'source': How was this triggered? (api, webhook, schedule, manual, ui) - 'caller_id': Who triggered it? (usr-abc123, sys-scheduler, webhook-stripe) - 'ip_address': Client IP address (for API/UI triggers) - 'user_agent': Client user agent (for API/UI triggers) - 'referrer': HTTP referrer (for UI triggers) - 'webhook_id': Webhook ID (for webhook triggers) - 'schedule_id': Schedule ID (for scheduled triggers) - 'timestamp': When was it triggered? (ISO 8601) Example (API trigger): trigger_metadata: { 'source': 'api' 'caller_id': 'usr-john-doe' 'ip_address': '203.0.113.42' 'user_agent': 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_15_7)' 'timestamp': '2025-01-11T14:30:22Z' } Example (webhook trigger): trigger_metadata: { 'source': 'webhook' 'webhook_id': 'whk-stripe-payment-received' 'webhook_source': 'stripe.com' 'event_type': 'payment_intent.succeeded' 'timestamp': '2025-01-11T14:30:22Z' } Use Cases: - Audit trail: Who triggered this execution and when? - Analytics: Which trigger sources are most common? - Debugging: Was this triggered by a webhook or manually? - Rate limiting: Limit executions per user/source"`

...
type SessionInput struct {
	...
	// Execution harness for this session. Determines which Temporal activity type is dispatched when an AgentExecution is created in this session: - LANGGRAPH (default): ExecuteGraphton activity -> Python/LangGraph worker - CURSOR: ExecuteCursor activity -> TypeScript/Cursor SDK worker The harness affects which tools the agent has, how conversation state is managed, available models, and billing tier. Once set and an execution has run, the harness is immutable — changing it would break conversation continuity since each harness owns its own state. When unspecified, defaults to HARNESS_LANGGRAPH.
	Harness string `json:"harness,omitempty" jsonschema:"Execution harness for this session. Determines which Temporal activity type is dispatched when an AgentExecution is created in this session: - LANGGRAPH (default): ExecuteGraphton activity -> Python/LangGraph worker - CURSOR: ExecuteCursor activity -> TypeScript/Cursor SDK worker The harness affects which tools the agent has, how conversation state is managed, available models, and billing tier. Once set and an execution has run, the harness is immutable — changing it would break conversation continuity since each harness owns its own state. When unspecified, defaults to HARNESS_LANGGRAPH. Allowed values: HARNESS_LANGGRAPH, HARNESS_CURSOR."`
}

...
type SessionInput struct {
	...
	// Deprecated: sandbox lifecycle is now managed at the Runner level via the stigmer.ai/sandbox-id metadata label on the Runner resource. Existing sessions may still have this field populated; new sessions should not set it.
	SandboxId string `json:"sandbox_id,omitempty" jsonschema:"Deprecated: sandbox lifecycle is now managed at the Runner level via the stigmer.ai/sandbox-id metadata label on the Runner resource. Existing sessions may still have this field populated; new sessions should not set it."`
	...
	// Runner that executes work for this session. When set, all executions in this session route to this runner's task queue. When empty, the platform auto-creates an ephemeral cloud runner on first execution and sets this field. For persistent runners (user-created via CLI/desktop), this is set by the session composer when the user picks their runner. For cloud executions, this is set automatically by the system. @internal The execution workflow reads this field to resolve the Temporal task queue for scheduling activities. When empty, the workflow calls the RunnerLauncher to spawn an ephemeral runner and populates this field.
	RunnerId string `json:"runner_id,omitempty" jsonschema:"Runner that executes work for this session. When set, all executions in this session route to this runner's task queue. When empty, the platform auto-creates an ephemeral cloud runner on first execution and sets this field. For persistent runners (user-created via CLI/desktop), this is set by the session composer when the user picks their runner. For cloud executions, this is set automatically by the system. @internal The execution workflow reads this field to resolve the Temporal task queue for scheduling activities. When empty, the workflow calls the RunnerLauncher to spawn an ephemeral runner and populates this field."`

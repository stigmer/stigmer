...
type GitRepoSourceInput struct {
	...
	// Controls whether the platform automatically creates a branch and pull request from the agent's file changes after execution completes. This is a platform-level workflow, not an agent-level decision. The agent focuses on making code changes; the platform packages them. Requires git credentials to be configured during workspace provisioning (GITHUB_TOKEN available in the execution environment). If credentials are not available, the write-back is silently skipped regardless of this setting. Default (UNSPECIFIED): no write-back, current behavior.
	WriteBackMode string `json:"write_back_mode,omitempty" jsonschema:"Controls whether the platform automatically creates a branch and pull request from the agent's file changes after execution completes. This is a platform-level workflow, not an agent-level decision. The agent focuses on making code changes; the platform packages them. Requires git credentials to be configured during workspace provisioning (GITHUB_TOKEN available in the execution environment). If credentials are not available, the write-back is silently skipped regardless of this setting. Default (UNSPECIFIED): no write-back, current behavior. Allowed values: GIT_WRITE_BACK_BRANCH_AND_PR."`
}

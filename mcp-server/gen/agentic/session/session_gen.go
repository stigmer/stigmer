...
type SessionInput struct {
	...
	// AgentRunner that executes work for this session. When set, all executions in this session route to this runner's task queue. When empty, the platform auto-creates an ephemeral cloud runner on first execution and sets this field. For persistent runners (user-created via CLI/desktop), this is set by the session composer when the user picks their runner. For cloud executions, this is set automatically by the system. @internal The execution workflow reads this field to resolve the Temporal task queue for scheduling Python activities. When empty, the workflow calls the RunnerLauncher to spawn an ephemeral runner and populates this field.
...
func (input *SessionInput) specToProto() (*sessionv1.SessionSpec, error) {
	...
	spec.AgentRunnerId = input.AgentRunnerId

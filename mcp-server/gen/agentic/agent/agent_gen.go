...
// McpServerUsage declares that this Agent uses a McpServer resource. The slug from mcp_server_ref becomes the identifier for SubAgent access. Design principle: Users already named their McpServer with a slug. We use that slug as the identifier - no extra naming required. Example YAML: mcp_server_usages: - mcp_server_ref: org: local kind: mcp_server slug: github enabled_tools: [search_code, get_file, create_pr] tool_approval_overrides: - tool_name: delete_repository requires_approval: false # Trust this agent - mcp_server_ref: org: acme-corp kind: mcp_server slug: internal-tools
...
// SubAgent defines a specialized agent that the parent can delegate to. SubAgents have restricted access to the parent's MCP servers. Permission Model: - SubAgent can only access MCP servers that parent has in mcp_server_usages - SubAgent tools must be a subset of parent's enabled tools (can restrict, not expand) - SubAgent skills can reference any Skill resource (independent of parent) Example YAML: sub_agents: - name: code-reviewer description: "Reviews code changes for quality and security" instructions: "You review code changes. Focus on security..." mcp_access: - mcp_server: github enabled_tools: [search_code, get_file] skill_refs: - org: local kind: skill slug: code-review-best-practices

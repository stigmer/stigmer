// Code generated by stigmer-codegen --target=mcp. DO NOT EDIT.
// Generated: 2026-02-28T03:53:20+05:30

package workflow

import (
	"encoding/json"
	"fmt"
	agentexecutionv1 "github.com/stigmer/stigmer/apis/stubs/go/ai/stigmer/agentic/agentexecution/v1"
	environmentv1 "github.com/stigmer/stigmer/apis/stubs/go/ai/stigmer/agentic/environment/v1"
	workflowv1 "github.com/stigmer/stigmer/apis/stubs/go/ai/stigmer/agentic/workflow/v1"
	tasks "github.com/stigmer/stigmer/apis/stubs/go/ai/stigmer/agentic/workflow/v1/tasks"
	"github.com/stigmer/stigmer/apis/stubs/go/ai/stigmer/commons/apiresource"
	"github.com/stigmer/stigmer/mcp-server/internal/convert"
	"google.golang.org/protobuf/encoding/protojson"
	"google.golang.org/protobuf/proto"
	structpb "google.golang.org/protobuf/types/known/structpb"
	timestamppb "google.golang.org/protobuf/types/known/timestamppb"
	"time"
)

// WorkflowSpec defines the complete specification of a workflow.
//
//	Follows the "kind + Struct" pattern from CloudResource (Planton Cloud).
//
//	This replaces the old `synthesized_yaml` field with structured proto definitions.
//	Each workflow task uses WorkflowTaskKind enum + google.protobuf.Struct for configuration,
//	providing maximum flexibility and extensibility.
type WorkflowInput struct {
	// Human-readable name of the resource.
	Name string `json:"name" jsonschema:"required,description=Human-readable name of the resource."`
	// URL-friendly identifier (lowercase alphanumeric with hyphens). Auto-generated from name if omitted.
	Slug string `json:"slug,omitempty" jsonschema:"description=URL-friendly identifier (lowercase alphanumeric with hyphens). Auto-generated from name if omitted."`
	// Organization that owns this resource (e.g. acme).
	Org string `json:"org" jsonschema:"required,description=Organization that owns this resource (e.g. acme)."`
	// Resource visibility: PRIVATE (default) or PUBLIC.
	Visibility string `json:"visibility,omitempty" jsonschema:"description=Resource visibility: PRIVATE (default) or PUBLIC."`
	// Key-value labels for organization and filtering.
	Labels map[string]string `json:"labels,omitempty" jsonschema:"description=Key-value labels for organization and filtering."`
	// Tags for categorization and discovery.
	Tags []string `json:"tags,omitempty" jsonschema:"description=Tags for categorization and discovery."`

	// Human-readable description for UI and marketplace display.
	Description string `json:"description,omitempty" jsonschema:"description=Human-readable description for UI and marketplace display."`
	// Workflow document metadata (DSL version, namespace, name, version).
	Document *WorkflowDocumentInput `json:"document" jsonschema:"required,description=Workflow document metadata (DSL version, namespace, name, version)."`
	// Ordered list of tasks that make up this workflow. Tasks execute sequentially unless fork/parallel is used.
	Tasks []WorkflowTaskInput `json:"tasks,omitempty" jsonschema:"description=Ordered list of tasks that make up this workflow. Tasks execute sequentially unless fork/parallel is used."`
	// Environment variables required by the workflow. Uses the shared EnvironmentSpec for consistent env var handling.
	EnvSpec *EnvironmentInput `json:"env_spec,omitempty" jsonschema:"description=Environment variables required by the workflow. Uses the shared EnvironmentSpec for consistent env var handling."`
}

// WorkflowDocument contains workflow metadata. Maps to the `document:` block in Zigflow DSL YAML.
type WorkflowDocumentInput struct {
	// DSL version (semver). Must be "1.0.0" for current Zigflow.
	Dsl string `json:"dsl,omitempty" jsonschema:"description=DSL version (semver). Must be '1.0.0' for current Zigflow."`
	// Workflow namespace (organization/categorization).
	Namespace string `json:"namespace" jsonschema:"required,description=Workflow namespace (organization/categorization)."`
	// Workflow name (unique identifier within namespace).
	Name string `json:"name" jsonschema:"required,description=Workflow name (unique identifier within namespace)."`
	// Workflow version (semver).
	Version string `json:"version" jsonschema:"required,description=Workflow version (semver)."`
	// Human-readable description.
	Description string `json:"description,omitempty" jsonschema:"description=Human-readable description."`
}

// ContextManagementConfig controls automatic context summarization behavior. Context summarization monitors token usage and automatically summarizes older conversation history when approaching the model's context window limit. This enables long-running agent conversations without hitting context limits. ## Default Behavior When not specified, defaults are derived from the Model Registry: - Each model has a configured context_window, trigger_threshold, and target_tokens - Summarization is enabled by default for all models - Economy-tier models (claude-haiku-4, gpt-4o-mini) are used for summarization ## Configuration Options - **disable_summarization**: Opt out of automatic summarization entirely - **custom_trigger_threshold**: Override when summarization triggers - **custom_target_tokens**: Override the target size after summarization ## Example YAML Default behavior (use model registry defaults): execution_config: model_name: "claude-sonnet-4.5" # context_management not specified = use defaults Disable summarization: execution_config: model_name: "claude-sonnet-4.5" context_management: disable_summarization: true Custom thresholds (tokens): execution_config: model_name: "claude-sonnet-4.5" context_management: custom_trigger_threshold: 100000 custom_target_tokens: 80000 @since Phase 3 (Context Summarization Architecture)
type ContextManagementConfigInput struct {
	// Disable automatic context summarization for this execution. When true, the agent will never trigger automatic summarization, even if the context window approaches the model's limit. Use cases: - Short-lived executions that won't approach context limits - Debugging context-related issues - Workflows that manage their own context externally Warning: Disabling summarization may cause execution failure if context exceeds model limits. Default: false (summarization enabled based on model defaults)
	DisableSummarization bool `json:"disable_summarization,omitempty" jsonschema:"description=Disable automatic context summarization for this execution. When true, the agent will never trigger automatic summarization, even if the context window approaches the model's limit. Use cases: - Short-lived executions that won't approach context limits - Debugging context-related issues - Workflows that manage their own context externally Warning: Disabling summarization may cause execution failure if context exceeds model limits. Default: false (summarization enabled based on model defaults)"`
	// Custom token threshold to trigger summarization. When the context token count exceeds this threshold, summarization is triggered to reduce context size. Set to 0 to use model default. The default trigger threshold from Model Registry is typically 90% of the model's context window (e.g., 180K for 200K context models). Must be greater than custom_target_tokens if both are specified. Default: 0 (use model default from Model Registry)
	CustomTriggerThreshold int32 `json:"custom_trigger_threshold,omitempty" jsonschema:"description=Custom token threshold to trigger summarization. When the context token count exceeds this threshold, summarization is triggered to reduce context size. Set to 0 to use model default. The default trigger threshold from Model Registry is typically 90% of the model's context window (e.g., 180K for 200K context models). Must be greater than custom_target_tokens if both are specified. Default: 0 (use model default from Model Registry)"`
	// Custom target token count after summarization. Summarization aims to reduce context to approximately this size. Set to 0 to use model default. The default target from Model Registry is typically 80% of the model's context window (e.g., 160K for 200K context models). Must be less than custom_trigger_threshold if both are specified. Default: 0 (use model default from Model Registry)
	CustomTargetTokens int32 `json:"custom_target_tokens,omitempty" jsonschema:"description=Custom target token count after summarization. Summarization aims to reduce context to approximately this size. Set to 0 to use model default. The default target from Model Registry is typically 80% of the model's context window (e.g., 160K for 200K context models). Must be less than custom_trigger_threshold if both are specified. Default: 0 (use model default from Model Registry)"`
}

// AgentExecutionConfig defines optional execution parameters for agent calls. These settings override the agent's default configuration for this specific invocation.
type AgentExecutionConfigInput struct {
	// LLM model to use for this invocation. Example: "claude-3-5-sonnet", "gpt-4", "claude-3-opus" Optional - uses agent's default model if not specified.
	Model string `json:"model,omitempty" jsonschema:"description=LLM model to use for this invocation. Example: 'claude-3-5-sonnet', 'gpt-4', 'claude-3-opus' Optional - uses agent's default model if not specified."`
	// Timeout for agent execution in seconds. Default: 300 (5 minutes) Optional.
	Timeout int32 `json:"timeout,omitempty" jsonschema:"description=Timeout for agent execution in seconds. Default: 300 (5 minutes) Optional."`
	// Temperature for LLM sampling (0.0 to 1.0). Lower = more deterministic, Higher = more creative Default: 0.7 Optional.
	Temperature float32 `json:"temperature,omitempty" jsonschema:"description=Temperature for LLM sampling (0.0 to 1.0). Lower = more deterministic, Higher = more creative Default: 0.7 Optional."`
	// Context management configuration for this agent invocation. Controls automatic summarization behavior for long-running conversations. When specified, overrides model defaults from the Model Registry. Use cases: - Disable summarization for short-lived agents - Custom thresholds for agents with specific context requirements - Fine-tune summarization behavior per workflow task Example YAML: config: model: "claude-sonnet-4.5" context_management: custom_trigger_threshold: 150000 custom_target_tokens: 120000 @since Phase 3 (Context Summarization Architecture)
	ContextManagement *ContextManagementConfigInput `json:"context_management,omitempty" jsonschema:"description=Context management configuration for this agent invocation. Controls automatic summarization behavior for long-running conversations. When specified, overrides model defaults from the Model Registry. Use cases: - Disable summarization for short-lived agents - Custom thresholds for agents with specific context requirements - Fine-tune summarization behavior per workflow task Example YAML: config: model: 'claude-sonnet-4.5' context_management: custom_trigger_threshold: 150000 custom_target_tokens: 120000 @since Phase 3 (Context Summarization Architecture)"`
}

// AgentCallTaskConfig defines the configuration for AGENT_CALL tasks. This enables workflows to invoke AI agents as tasks, delegating complex operations to specialized agents with their own skills and context. The agent is referenced by org/slug format (e.g., "stigmer/code-reviewer"). Resolution order: 1. If org is specified: look in that org's agents 2. If org is empty: use the workflow's org 3. Before external lookup, check manifest (current deployment) The workflow's execution context (environment variables, secrets) is passed to the agent invocation, allowing agents to access workflow state. YAML Example: - analyze: call: agent with: agent: "code-reviewer" # Uses workflow's org # OR agent: "stigmer/code-reviewer" # Explicit org reference message: "Review this code: ${ $context.fetchCode.body }" env: GITHUB_TOKEN: "${ .secrets.GH_TOKEN }" config: model: "claude-3-5-sonnet" timeout: 300 Reference: design doc at stigmer/_cursor/add-agent-config-to-workflow.md
type AgentCallTaskConfigInput struct {
	// Agent reference in "org/slug" or "slug" format. - "slug" only: uses the workflow's organization - "org/slug": explicit organization reference Examples: "code-reviewer", "stigmer/code-reviewer", "acme/data-analyst" Required field.
	Agent string `json:"agent" jsonschema:"required,description=Agent reference in 'org/slug' or 'slug' format. - 'slug' only: uses the workflow's organization - 'org/slug': explicit organization reference Examples: 'code-reviewer', 'stigmer/code-reviewer', 'acme/data-analyst' Required field."`
	// Explicit organization for agent resolution. Optional. If empty, the org is parsed from the agent field or defaults to workflow's org. Use this when you need to override the parsed org.
	Org string `json:"org,omitempty" jsonschema:"description=Explicit organization for agent resolution. Optional. If empty, the org is parsed from the agent field or defaults to workflow's org. Use this when you need to override the parsed org."`
	// Instructions/prompt to send to the agent. Supports interpolation of workflow variables using JQ expressions. Example: "Analyze this code: ${ $context.fetchCode.body }" Required field.
	Message string `json:"message" jsonschema:"required,description=Instructions/prompt to send to the agent. Supports interpolation of workflow variables using JQ expressions. Example: 'Analyze this code: ${ $context.fetchCode.body }' Required field."`
	// Runtime environment variables to pass to the agent. Values can be literal strings or JQ expressions that reference workflow context or secrets. Example: {"GITHUB_TOKEN": "${ .secrets.GH_TOKEN }"} Optional.
	Env map[string]string `json:"env,omitempty" jsonschema:"description=Runtime environment variables to pass to the agent. Values can be literal strings or JQ expressions that reference workflow context or secrets. Example: {'GITHUB_TOKEN': '${ .secrets.GH_TOKEN }'} Optional."`
	// Execution configuration for the agent invocation. Optional - defaults are applied if not specified.
	Config *AgentExecutionConfigInput `json:"config,omitempty" jsonschema:"description=Execution configuration for the agent invocation. Optional - defaults are applied if not specified."`
}

// CallActivityTaskConfig defines the configuration for CALL_ACTIVITY tasks. CALL_ACTIVITY tasks execute Temporal activities. YAML Example: - taskName: call: activity with: activity: "ProcessDataActivity" input: data: ${ .data } Reference: zigflow-dsl-pattern-catalog.md - Task Type 10
type CallActivityTaskConfigInput struct {
	// Activity name to execute. Must match registered Temporal activity.
	Activity string `json:"activity" jsonschema:"required,description=Activity name to execute. Must match registered Temporal activity."`
	// Activity input (optional). Can be any JSON structure. Supports expressions in string values.
	Input map[string]any `json:"input,omitempty" jsonschema:"description=Activity input (optional). Can be any JSON structure. Supports expressions in string values."`
}

// ForTaskConfig defines the configuration for FOR tasks. FOR tasks iterate over collections, executing tasks for each item. YAML Example: - taskName: for: each: item in: ${ $data.items } do: - processItem: call: http with: method: POST body: item: ${ $data.item } index: ${ $data.index } Reference: zigflow-dsl-pattern-catalog.md - Task Type 4
type ForTaskConfigInput struct {
	// Variable name for each item in the iteration. Accessible via ${ $data.item } in expressions.
	Each string `json:"each" jsonschema:"required,description=Variable name for each item in the iteration. Accessible via ${ $data.item } in expressions."`
	// Expression evaluating to the collection to iterate over. Example: "${ $data.items }" or "${ [1, 2, 3] }"
	In string `json:"in" jsonschema:"required,description=Expression evaluating to the collection to iterate over. Example: '${ $data.items }' or '${ [1, 2, 3] }'"`
	// Tasks to execute for each iteration. Loop variables available: ${ $data.item }, ${ $data.index }
	Do []WorkflowTaskInput `json:"do,omitempty" jsonschema:"description=Tasks to execute for each iteration. Loop variables available: ${ $data.item }, ${ $data.index }"`
}

// ForkBranch defines a single branch in parallel execution.
type ForkBranchInput struct {
	// Branch name/identifier.
	Name string `json:"name" jsonschema:"required,description=Branch name/identifier."`
	// Tasks to execute in this branch.
	Do []WorkflowTaskInput `json:"do,omitempty" jsonschema:"description=Tasks to execute in this branch."`
}

// ForkTaskConfig defines the configuration for FORK tasks. FORK tasks execute multiple branches in parallel. YAML Example: - taskName: fork: branches: - branch1: do: - task1: call: http with: method: POST endpoint: uri: https://api.example.com/branch1 - branch2: do: - task2: call: http Reference: zigflow-dsl-pattern-catalog.md - Task Type 5
type ForkTaskConfigInput struct {
	// Branches to execute in parallel (at least 2 required).
	Branches []ForkBranchInput `json:"branches,omitempty" jsonschema:"description=Branches to execute in parallel (at least 2 required)."`
	// If true, first branch to complete wins (race mode). If false, all branches must complete (default).
	Compete bool `json:"compete,omitempty" jsonschema:"description=If true, first branch to complete wins (race mode). If false, all branches must complete (default)."`
}

// GrpcCallTaskConfig defines the configuration for GRPC_CALL tasks. GRPC_CALL tasks make gRPC requests to external services. YAML Example: - taskName: call: grpc with: service: "com.example.UserService" method: "GetUser" request: userId: ${ .userId } Reference: zigflow-dsl-pattern-catalog.md - Task Type 9
type GrpcCallTaskConfigInput struct {
	// Fully qualified service name. Example: "com.example.UserService"
	Service string `json:"service" jsonschema:"required,description=Fully qualified service name. Example: 'com.example.UserService'"`
	// Method name to call. Example: "GetUser"
	Method string `json:"method" jsonschema:"required,description=Method name to call. Example: 'GetUser'"`
	// Request message (optional). Can be any JSON structure matching the proto schema. Supports expressions in string values.
	Request map[string]any `json:"request,omitempty" jsonschema:"description=Request message (optional). Can be any JSON structure matching the proto schema. Supports expressions in string values."`
}

// HttpEndpoint defines the HTTP endpoint to call.
type HttpEndpointInput struct {
	// URI of the endpoint. Can contain expressions: "https://api.example.com/${.resource}"
	Uri string `json:"uri" jsonschema:"required,description=URI of the endpoint. Can contain expressions: 'https://api.example.com/${.resource}'"`
}

// HttpCallTaskConfig defines the configuration for HTTP_CALL tasks. HTTP_CALL tasks make HTTP requests (GET, POST, PUT, DELETE, PATCH). YAML Example: - taskName: call: http with: method: POST endpoint: uri: https://api.example.com/data headers: Authorization: "Bearer ${TOKEN}" body: field1: value Reference: zigflow-dsl-pattern-catalog.md - Task Type 2
type HttpCallTaskConfigInput struct {
	// HTTP method (GET, POST, PUT, DELETE, PATCH).
	Method string `json:"method" jsonschema:"required,enum=GET|POST|PUT|DELETE|PATCH,description=HTTP method (GET, POST, PUT, DELETE, PATCH)."`
	// HTTP endpoint configuration.
	Endpoint *HttpEndpointInput `json:"endpoint" jsonschema:"required,description=HTTP endpoint configuration."`
	// HTTP headers (optional). Values can contain expressions: "Bearer ${TOKEN}"
	Headers map[string]string `json:"headers,omitempty" jsonschema:"description=HTTP headers (optional). Values can contain expressions: 'Bearer ${TOKEN}'"`
	// Request body (optional). Can be any JSON structure. Supports expressions in string values.
	Body map[string]any `json:"body,omitempty" jsonschema:"description=Request body (optional). Can be any JSON structure. Supports expressions in string values."`
	// Request timeout in seconds (optional, default: 30).
	TimeoutSeconds int32 `json:"timeout_seconds,omitempty" jsonschema:"description=Request timeout in seconds (optional, default: 30)."`
}

// SignalSpec defines a signal/event to listen for.
type SignalInput struct {
	// Signal identifier.
	Id string `json:"id" jsonschema:"required,description=Signal identifier."`
	// Signal type: - "signal": Temporal signal - "query": Temporal query - "update": Temporal update
	Type string `json:"type" jsonschema:"required,enum=signal|query|update,description=Signal type: - 'signal': Temporal signal - 'query': Temporal query - 'update': Temporal update"`
}

// ListenTo defines what signals to listen for.
type ListenToInput struct {
	// Listening mode: - "one": Wait for any one signal - "all": Wait for all signals
	Mode string `json:"mode" jsonschema:"required,enum=one|all,description=Listening mode: - 'one': Wait for any one signal - 'all': Wait for all signals"`
	// Signals to listen for.
	Signals []SignalInput `json:"signals,omitempty" jsonschema:"description=Signals to listen for."`
}

// ListenTaskConfig defines the configuration for LISTEN tasks. LISTEN tasks wait for external signals/events (Temporal signals). YAML Example: - taskName: listen: to: one: with: id: approval_signal type: signal Reference: zigflow-dsl-pattern-catalog.md - Task Type 7
type ListenTaskConfigInput struct {
	// Signal listening configuration.
	To *ListenToInput `json:"to" jsonschema:"required,description=Signal listening configuration."`
}

// RaiseTaskConfig defines the configuration for RAISE tasks. RAISE tasks raise errors/exceptions, terminating workflow execution. YAML Example: - taskName: raise: error: ValidationError message: ${ .errorMessage } Reference: zigflow-dsl-pattern-catalog.md - Task Type 11
type RaiseTaskConfigInput struct {
	// Error type/name.
	Error string `json:"error" jsonschema:"required,description=Error type/name."`
	// Error message. Can contain expressions: "${ .errorMessage }"
	Message string `json:"message" jsonschema:"required,description=Error message. Can contain expressions: '${ .errorMessage }'"`
}

// RunTaskConfig defines the configuration for RUN tasks. RUN tasks execute sub-workflows (Temporal child workflows). YAML Example: - taskName: run: workflow: "sub-workflow-name" input: data: ${ .data } Reference: zigflow-dsl-pattern-catalog.md - Task Type 12
type RunTaskConfigInput struct {
	// Sub-workflow name/identifier to execute.
	Workflow string `json:"workflow" jsonschema:"required,description=Sub-workflow name/identifier to execute."`
	// Sub-workflow input (optional). Can be any JSON structure. Supports expressions in string values.
	Input map[string]any `json:"input,omitempty" jsonschema:"description=Sub-workflow input (optional). Can be any JSON structure. Supports expressions in string values."`
}

// SetTaskConfig defines the configuration for SET tasks. SET tasks assign variables in workflow state. YAML Example: - taskName: set: variable1: value variable2: ${ expression } computed: ${ .a + .b } Reference: zigflow-dsl-pattern-catalog.md - Task Type 1
type SetTaskConfigInput struct {
	// Variables to set in workflow state. Keys are variable names, values can be literals or expressions. Expressions use ${...} syntax, e.g., "${.a + .b}" or "${now}"
	Variables map[string]string `json:"variables" jsonschema:"required,description=Variables to set in workflow state. Keys are variable names, values can be literals or expressions. Expressions use ${...} syntax, e.g., '${.a + .b}' or '${now}'"`
}

// SwitchCase defines a single case in a switch statement.
type SwitchCaseInput struct {
	// Case name/identifier.
	Name string `json:"name" jsonschema:"required,description=Case name/identifier."`
	// Condition expression (optional). If empty, this is the default case. Example: "${ $context.value > 5 }"
	When string `json:"when,omitempty" jsonschema:"description=Condition expression (optional). If empty, this is the default case. Example: '${ $context.value > 5 }'"`
	// Target task name to execute if condition matches.
	Then string `json:"then" jsonschema:"required,description=Target task name to execute if condition matches."`
}

// SwitchTaskConfig defines the configuration for SWITCH tasks. SWITCH tasks provide conditional branching based on expressions. YAML Example: - taskName: switch: - case1: when: ${ $context.value > 5 } then: highValueTask - defaultCase: then: unknownTask Reference: zigflow-dsl-pattern-catalog.md - Task Type 3
type SwitchTaskConfigInput struct {
	// List of switch cases (at least one required). Cases are evaluated in order. First matching case executes. If no "when" is specified, the case acts as default.
	Cases []SwitchCaseInput `json:"cases,omitempty" jsonschema:"description=List of switch cases (at least one required). Cases are evaluated in order. First matching case executes. If no 'when' is specified, the case acts as default."`
}

// CatchBlock defines error handling logic.
type CatchBlockInput struct {
	// Variable name to store the error (optional). Accessible via ${ .error } in catch tasks.
	As string `json:"as,omitempty" jsonschema:"description=Variable name to store the error (optional). Accessible via ${ .error } in catch tasks."`
	// Tasks to execute when error is caught.
	Do []WorkflowTaskInput `json:"do,omitempty" jsonschema:"description=Tasks to execute when error is caught."`
}

// TryTaskConfig defines the configuration for TRY tasks. TRY tasks provide try/catch error handling. YAML Example: - taskName: try: - attemptTask: call: http with: method: POST endpoint: uri: https://api.example.com/flaky catch: as: error do: - errorHandler: call: http with: body: error: ${ .error } Reference: zigflow-dsl-pattern-catalog.md - Task Type 6
type TryTaskConfigInput struct {
	// Tasks to attempt (at least one required). If any task fails, execution jumps to catch block.
	Try []WorkflowTaskInput `json:"try,omitempty" jsonschema:"description=Tasks to attempt (at least one required). If any task fails, execution jumps to catch block."`
	// Catch block for error handling (optional). If not provided, errors propagate to parent.
	Catch *CatchBlockInput `json:"catch,omitempty" jsonschema:"description=Catch block for error handling (optional). If not provided, errors propagate to parent."`
}

// Duration represents a relative time period. Fields are additive: { days: 1, hours: 12 } equals 36 hours total. At least one field must be non-zero. Examples: - 1 week: { days: 7 } - 2.5 hours: { hours: 2, minutes: 30 } - 90 seconds: { minutes: 1, seconds: 30 }
type DurationInput struct {
	// Number of days to wait (24 hours each).
	Days uint32 `json:"days,omitempty" jsonschema:"description=Number of days to wait (24 hours each)."`
	// Number of hours to wait.
	Hours uint32 `json:"hours,omitempty" jsonschema:"description=Number of hours to wait."`
	// Number of minutes to wait.
	Minutes uint32 `json:"minutes,omitempty" jsonschema:"description=Number of minutes to wait."`
	// Number of seconds to wait.
	Seconds uint32 `json:"seconds,omitempty" jsonschema:"description=Number of seconds to wait."`
	// Number of milliseconds to wait.
	Milliseconds uint32 `json:"milliseconds,omitempty" jsonschema:"description=Number of milliseconds to wait."`
}

// WaitTaskConfig defines the configuration for WAIT tasks. WAIT tasks pause workflow execution using Temporal timers. Supports both relative durations ("wait 1 week") and absolute timestamps ("wait until March 1st 9am"). YAML Examples: Relative duration: - waitForApproval: wait: duration: days: 7 Absolute timestamp: - waitUntilMarketOpen: wait: until: "2026-03-02T09:30:00Z" Reference: zigflow-dsl-pattern-catalog.md - Task Type 8
type WaitTaskConfigInput struct {
	// Relative duration to wait from now. Use this when you want to wait for a specific amount of time regardless of when the workflow reaches this task. Example: { days: 7 } = wait 1 week from now
	Duration *DurationInput `json:"duration,omitempty" jsonschema:"description=Relative duration to wait from now. Use this when you want to wait for a specific amount of time regardless of when the workflow reaches this task. Example: { days: 7 } = wait 1 week from now"`
	// Absolute timestamp to wait until. Use this when you want to wait until a specific point in time. If the timestamp is in the past, the task completes immediately. Example: "2026-03-01T09:00:00Z" = wait until March 1st, 2026 at 9am UTC
	Until string `json:"until,omitempty" jsonschema:"description=Absolute timestamp to wait until. Use this when you want to wait until a specific point in time. If the timestamp is in the past, the task completes immediately. Example: '2026-03-01T09:00:00Z' = wait until March 1st, 2026 at 9am UTC"`
}

// Export defines how to save task output to context. Maps to the `export:` block in Zigflow DSL. Examples: - {"as": "${.}"} - Export entire output - {"as": "${.fieldName}"} - Export specific field - {"as": "${$context + {taskName: .}}"} - Merge into context
type ExportInput struct {
	// Expression defining how to export output. Uses Zigflow expression syntax: ${...}
	As string `json:"as,omitempty" jsonschema:"description=Expression defining how to export output. Uses Zigflow expression syntax: ${...}"`
}

// FlowControl defines which task executes next. Maps to the `then:` directive in Zigflow DSL. Examples: - {"then": "nextTaskName"} - Jump to specific task - {"then": "end"} - Terminate workflow - Not set - Continue to next task in sequence (default)
type FlowControlInput struct {
	// Target task name or "end" to terminate workflow.
	Then string `json:"then,omitempty" jsonschema:"description=Target task name or 'end' to terminate workflow."`
}

// A single workflow task. Set kind to the task type and populate exactly one matching config field (e.g. kind='http_call' -> set the http_call field).
type WorkflowTaskInput struct {
	// Task name/identifier (must be unique within workflow).
	Name string `json:"name" jsonschema:"required,description=Task name/identifier (must be unique within workflow)."`
	// Task type. Set the matching config field (e.g. kind='http_call' -> populate http_call).
	Kind string `json:"kind" jsonschema:"required,enum=set_vars|http_call|grpc_call|activity_call|switch_case|for_each|fork|try_catch|listen|wait|raise_error|run_workflow|agent_call,description=Task type. Set the matching config field (e.g. kind='http_call' -> populate http_call)."`
	// Required when kind='agent_call'. AgentCallTaskConfig defines the configuration for AGENT_CALL tasks. This enables workflows to invoke AI agents as tasks, delegating complex operations to specialized agents with their own skills and context. The agent is referenced by org/slug format (e.g., "stigmer/code-reviewer"). Resolution order: 1. If org is specified: look in that org's agents 2. If org is empty: use the workflow's org 3. Before external lookup, check manifest (current deployment) The workflow's execution context (environment variables, secrets) is passed to the agent invocation, allowing agents to access workflow state. YAML Example: - analyze: call: agent with: agent: "code-reviewer" # Uses workflow's org # OR agent: "stigmer/code-reviewer" # Explicit org reference message: "Review this code: ${ $context.fetchCode.body }" env: GITHUB_TOKEN: "${ .secrets.GH_TOKEN }" config: model: "claude-3-5-sonnet" timeout: 300 Reference: design doc at stigmer/_cursor/add-agent-config-to-workflow.md
	AgentCall *AgentCallTaskConfigInput `json:"agent_call,omitempty" jsonschema:"description=Required when kind='agent_call'. AgentCallTaskConfig defines the configuration for AGENT_CALL tasks. This enables workflows to invoke AI agents as tasks, delegating complex operations to specialized agents with their own skills and context. The agent is referenced by org/slug format (e.g., 'stigmer/code-reviewer'). Resolution order: 1. If org is specified: look in that org's agents 2. If org is empty: use the workflow's org 3. Before external lookup, check manifest (current deployment) The workflow's execution context (environment variables, secrets) is passed to the agent invocation, allowing agents to access workflow state. YAML Example: - analyze: call: agent with: agent: 'code-reviewer' # Uses workflow's org # OR agent: 'stigmer/code-reviewer' # Explicit org reference message: 'Review this code: ${ $context.fetchCode.body }' env: GITHUB_TOKEN: '${ .secrets.GH_TOKEN }' config: model: 'claude-3-5-sonnet' timeout: 300 Reference: design doc at stigmer/_cursor/add-agent-config-to-workflow.md"`
	// Required when kind='activity_call'. CallActivityTaskConfig defines the configuration for CALL_ACTIVITY tasks. CALL_ACTIVITY tasks execute Temporal activities. YAML Example: - taskName: call: activity with: activity: "ProcessDataActivity" input: data: ${ .data } Reference: zigflow-dsl-pattern-catalog.md - Task Type 10
	ActivityCall *CallActivityTaskConfigInput `json:"activity_call,omitempty" jsonschema:"description=Required when kind='activity_call'. CallActivityTaskConfig defines the configuration for CALL_ACTIVITY tasks. CALL_ACTIVITY tasks execute Temporal activities. YAML Example: - taskName: call: activity with: activity: 'ProcessDataActivity' input: data: ${ .data } Reference: zigflow-dsl-pattern-catalog.md - Task Type 10"`
	// Required when kind='for_each'. ForTaskConfig defines the configuration for FOR tasks. FOR tasks iterate over collections, executing tasks for each item. YAML Example: - taskName: for: each: item in: ${ $data.items } do: - processItem: call: http with: method: POST body: item: ${ $data.item } index: ${ $data.index } Reference: zigflow-dsl-pattern-catalog.md - Task Type 4
	ForEach *ForTaskConfigInput `json:"for_each,omitempty" jsonschema:"description=Required when kind='for_each'. ForTaskConfig defines the configuration for FOR tasks. FOR tasks iterate over collections, executing tasks for each item. YAML Example: - taskName: for: each: item in: ${ $data.items } do: - processItem: call: http with: method: POST body: item: ${ $data.item } index: ${ $data.index } Reference: zigflow-dsl-pattern-catalog.md - Task Type 4"`
	// Required when kind='fork'. ForkTaskConfig defines the configuration for FORK tasks. FORK tasks execute multiple branches in parallel. YAML Example: - taskName: fork: branches: - branch1: do: - task1: call: http with: method: POST endpoint: uri: https://api.example.com/branch1 - branch2: do: - task2: call: http Reference: zigflow-dsl-pattern-catalog.md - Task Type 5
	Fork *ForkTaskConfigInput `json:"fork,omitempty" jsonschema:"description=Required when kind='fork'. ForkTaskConfig defines the configuration for FORK tasks. FORK tasks execute multiple branches in parallel. YAML Example: - taskName: fork: branches: - branch1: do: - task1: call: http with: method: POST endpoint: uri: https://api.example.com/branch1 - branch2: do: - task2: call: http Reference: zigflow-dsl-pattern-catalog.md - Task Type 5"`
	// Required when kind='grpc_call'. GrpcCallTaskConfig defines the configuration for GRPC_CALL tasks. GRPC_CALL tasks make gRPC requests to external services. YAML Example: - taskName: call: grpc with: service: "com.example.UserService" method: "GetUser" request: userId: ${ .userId } Reference: zigflow-dsl-pattern-catalog.md - Task Type 9
	GrpcCall *GrpcCallTaskConfigInput `json:"grpc_call,omitempty" jsonschema:"description=Required when kind='grpc_call'. GrpcCallTaskConfig defines the configuration for GRPC_CALL tasks. GRPC_CALL tasks make gRPC requests to external services. YAML Example: - taskName: call: grpc with: service: 'com.example.UserService' method: 'GetUser' request: userId: ${ .userId } Reference: zigflow-dsl-pattern-catalog.md - Task Type 9"`
	// Required when kind='http_call'. HttpCallTaskConfig defines the configuration for HTTP_CALL tasks. HTTP_CALL tasks make HTTP requests (GET, POST, PUT, DELETE, PATCH). YAML Example: - taskName: call: http with: method: POST endpoint: uri: https://api.example.com/data headers: Authorization: "Bearer ${TOKEN}" body: field1: value Reference: zigflow-dsl-pattern-catalog.md - Task Type 2
	HttpCall *HttpCallTaskConfigInput `json:"http_call,omitempty" jsonschema:"description=Required when kind='http_call'. HttpCallTaskConfig defines the configuration for HTTP_CALL tasks. HTTP_CALL tasks make HTTP requests (GET, POST, PUT, DELETE, PATCH). YAML Example: - taskName: call: http with: method: POST endpoint: uri: https://api.example.com/data headers: Authorization: 'Bearer ${TOKEN}' body: field1: value Reference: zigflow-dsl-pattern-catalog.md - Task Type 2"`
	// Required when kind='listen'. ListenTaskConfig defines the configuration for LISTEN tasks. LISTEN tasks wait for external signals/events (Temporal signals). YAML Example: - taskName: listen: to: one: with: id: approval_signal type: signal Reference: zigflow-dsl-pattern-catalog.md - Task Type 7
	Listen *ListenTaskConfigInput `json:"listen,omitempty" jsonschema:"description=Required when kind='listen'. ListenTaskConfig defines the configuration for LISTEN tasks. LISTEN tasks wait for external signals/events (Temporal signals). YAML Example: - taskName: listen: to: one: with: id: approval_signal type: signal Reference: zigflow-dsl-pattern-catalog.md - Task Type 7"`
	// Required when kind='raise_error'. RaiseTaskConfig defines the configuration for RAISE tasks. RAISE tasks raise errors/exceptions, terminating workflow execution. YAML Example: - taskName: raise: error: ValidationError message: ${ .errorMessage } Reference: zigflow-dsl-pattern-catalog.md - Task Type 11
	RaiseError *RaiseTaskConfigInput `json:"raise_error,omitempty" jsonschema:"description=Required when kind='raise_error'. RaiseTaskConfig defines the configuration for RAISE tasks. RAISE tasks raise errors/exceptions, terminating workflow execution. YAML Example: - taskName: raise: error: ValidationError message: ${ .errorMessage } Reference: zigflow-dsl-pattern-catalog.md - Task Type 11"`
	// Required when kind='run_workflow'. RunTaskConfig defines the configuration for RUN tasks. RUN tasks execute sub-workflows (Temporal child workflows). YAML Example: - taskName: run: workflow: "sub-workflow-name" input: data: ${ .data } Reference: zigflow-dsl-pattern-catalog.md - Task Type 12
	RunWorkflow *RunTaskConfigInput `json:"run_workflow,omitempty" jsonschema:"description=Required when kind='run_workflow'. RunTaskConfig defines the configuration for RUN tasks. RUN tasks execute sub-workflows (Temporal child workflows). YAML Example: - taskName: run: workflow: 'sub-workflow-name' input: data: ${ .data } Reference: zigflow-dsl-pattern-catalog.md - Task Type 12"`
	// Required when kind='set_vars'. SetTaskConfig defines the configuration for SET tasks. SET tasks assign variables in workflow state. YAML Example: - taskName: set: variable1: value variable2: ${ expression } computed: ${ .a + .b } Reference: zigflow-dsl-pattern-catalog.md - Task Type 1
	SetVars *SetTaskConfigInput `json:"set_vars,omitempty" jsonschema:"description=Required when kind='set_vars'. SetTaskConfig defines the configuration for SET tasks. SET tasks assign variables in workflow state. YAML Example: - taskName: set: variable1: value variable2: ${ expression } computed: ${ .a + .b } Reference: zigflow-dsl-pattern-catalog.md - Task Type 1"`
	// Required when kind='switch_case'. SwitchTaskConfig defines the configuration for SWITCH tasks. SWITCH tasks provide conditional branching based on expressions. YAML Example: - taskName: switch: - case1: when: ${ $context.value > 5 } then: highValueTask - defaultCase: then: unknownTask Reference: zigflow-dsl-pattern-catalog.md - Task Type 3
	SwitchCase *SwitchTaskConfigInput `json:"switch_case,omitempty" jsonschema:"description=Required when kind='switch_case'. SwitchTaskConfig defines the configuration for SWITCH tasks. SWITCH tasks provide conditional branching based on expressions. YAML Example: - taskName: switch: - case1: when: ${ $context.value > 5 } then: highValueTask - defaultCase: then: unknownTask Reference: zigflow-dsl-pattern-catalog.md - Task Type 3"`
	// Required when kind='try_catch'. TryTaskConfig defines the configuration for TRY tasks. TRY tasks provide try/catch error handling. YAML Example: - taskName: try: - attemptTask: call: http with: method: POST endpoint: uri: https://api.example.com/flaky catch: as: error do: - errorHandler: call: http with: body: error: ${ .error } Reference: zigflow-dsl-pattern-catalog.md - Task Type 6
	TryCatch *TryTaskConfigInput `json:"try_catch,omitempty" jsonschema:"description=Required when kind='try_catch'. TryTaskConfig defines the configuration for TRY tasks. TRY tasks provide try/catch error handling. YAML Example: - taskName: try: - attemptTask: call: http with: method: POST endpoint: uri: https://api.example.com/flaky catch: as: error do: - errorHandler: call: http with: body: error: ${ .error } Reference: zigflow-dsl-pattern-catalog.md - Task Type 6"`
	// Required when kind='wait'. WaitTaskConfig defines the configuration for WAIT tasks. WAIT tasks pause workflow execution using Temporal timers. Supports both relative durations ("wait 1 week") and absolute timestamps ("wait until March 1st 9am"). YAML Examples: Relative duration: - waitForApproval: wait: duration: days: 7 Absolute timestamp: - waitUntilMarketOpen: wait: until: "2026-03-02T09:30:00Z" Reference: zigflow-dsl-pattern-catalog.md - Task Type 8
	Wait *WaitTaskConfigInput `json:"wait,omitempty" jsonschema:"description=Required when kind='wait'. WaitTaskConfig defines the configuration for WAIT tasks. WAIT tasks pause workflow execution using Temporal timers. Supports both relative durations ('wait 1 week') and absolute timestamps ('wait until March 1st 9am'). YAML Examples: Relative duration: - waitForApproval: wait: duration: days: 7 Absolute timestamp: - waitUntilMarketOpen: wait: until: '2026-03-02T09:30:00Z' Reference: zigflow-dsl-pattern-catalog.md - Task Type 8"`
	// Export configuration (how to save task output to context). Optional - if not set, output is not saved.
	Export *ExportInput `json:"export,omitempty" jsonschema:"description=Export configuration (how to save task output to context). Optional - if not set, output is not saved."`
	// Flow control (which task executes next). Optional - if not set, continues to next task in sequence.
	Flow *FlowControlInput `json:"flow,omitempty" jsonschema:"description=Flow control (which task executes next). Optional - if not set, continues to next task in sequence."`
}

// EnvironmentValue represents a single configuration or secret value.
type EnvironmentValue struct {
	// The actual value. - If is_secret=true: This value is encrypted at rest and redacted in logs - If is_secret=false: This value is stored as plaintext Note: Value can be empty when defining environment variables in specs. Actual values are typically provided at runtime during execution.
	Value string `json:"value,omitempty" jsonschema:"description=The actual value. - If is_secret=true: This value is encrypted at rest and redacted in logs - If is_secret=false: This value is stored as plaintext Note: Value can be empty when defining environment variables in specs. Actual values are typically provided at runtime during execution."`
	// Whether this value should be treated as a secret. When true: - Value is encrypted at rest - Value is redacted in logs - Value requires special permissions to read When false: - Value is stored as plaintext - Value is visible in audit logs
	IsSecret bool `json:"is_secret,omitempty" jsonschema:"description=Whether this value should be treated as a secret. When true: - Value is encrypted at rest - Value is redacted in logs - Value requires special permissions to read When false: - Value is stored as plaintext - Value is visible in audit logs"`
	// Optional description for documentation. Example: "AWS access key for S3 bucket access"
	Description string `json:"description,omitempty" jsonschema:"description=Optional description for documentation. Example: 'AWS access key for S3 bucket access'"`
}

// EnvironmentSpec defines a collection of configuration and secrets. Created before AgentInstance or WorkflowInstance, referenced during instance creation.
type EnvironmentInput struct {
	// Human-readable description of this environment. Example: "Production AWS credentials for deployment"
	Description string `json:"description,omitempty" jsonschema:"description=Human-readable description of this environment. Example: 'Production AWS credentials for deployment'"`
	// Key-value pairs containing both configuration and secrets. Each value includes a flag indicating whether it's a secret. Example: {"AWS_REGION": {value: "us-west-2", is_secret: false}, "AWS_ACCESS_KEY_ID": {value: "AKIA...", is_secret: true}}
	Data map[string]*EnvironmentValue `json:"data,omitempty" jsonschema:"description=Key-value pairs containing both configuration and secrets. Each value includes a flag indicating whether it's a secret. Example: {'AWS_REGION': {value: 'us-west-2', is_secret: false}, 'AWS_ACCESS_KEY_ID': {value: 'AKIA...', is_secret: true}}"`
}

func protoToStruct(msg proto.Message) (*structpb.Struct, error) {
	data, err := protojson.Marshal(msg)
	if err != nil {
		return nil, fmt.Errorf("protojson marshal: %w", err)
	}
	var m map[string]any
	if err := json.Unmarshal(data, &m); err != nil {
		return nil, fmt.Errorf("json unmarshal: %w", err)
	}
	return structpb.NewStruct(m)
}

// ToProto converts the flat MCP input into a fully-formed Workflow proto message.
func (input *WorkflowInput) ToProto() (*workflowv1.Workflow, error) {
	slug := input.Slug
	if slug == "" {
		slug = convert.GenerateSlug(input.Name)
	}

	spec, err := input.specToProto()
	if err != nil {
		return nil, err
	}

	return &workflowv1.Workflow{
		ApiVersion: "agentic.stigmer.ai/v1",
		Kind:       "Workflow",
		Metadata: &apiresource.ApiResourceMetadata{
			Name:       input.Name,
			Slug:       slug,
			Org:        input.Org,
			Visibility: convert.VisibilityFromString(input.Visibility),
			Labels:     input.Labels,
			Tags:       input.Tags,
		},
		Spec: spec,
	}, nil
}

func (input *WorkflowInput) specToProto() (*workflowv1.WorkflowSpec, error) {
	spec := &workflowv1.WorkflowSpec{}

	spec.Description = input.Description
	if input.Document != nil {
		v, err := input.Document.toProto()
		if err != nil {
			return nil, err
		}
		spec.Document = v
	}
	for _, item := range input.Tasks {
		v, err := item.toProto()
		if err != nil {
			return nil, err
		}
		spec.Tasks = append(spec.Tasks, v)
	}
	if input.EnvSpec != nil {
		v, err := input.EnvSpec.toProto()
		if err != nil {
			return nil, err
		}
		spec.EnvSpec = v
	}
	return spec, nil
}

func (input *WorkflowDocumentInput) toProto() (*workflowv1.WorkflowDocument, error) {
	result := &workflowv1.WorkflowDocument{}

	result.Dsl = input.Dsl
	result.Namespace = input.Namespace
	result.Name = input.Name
	result.Version = input.Version
	result.Description = input.Description
	return result, nil
}

func (input *ContextManagementConfigInput) toProto() (*agentexecutionv1.ContextManagementConfig, error) {
	result := &agentexecutionv1.ContextManagementConfig{}

	result.DisableSummarization = input.DisableSummarization
	result.CustomTriggerThreshold = input.CustomTriggerThreshold
	result.CustomTargetTokens = input.CustomTargetTokens
	return result, nil
}

func (input *AgentExecutionConfigInput) toProto() (*tasks.AgentExecutionConfig, error) {
	result := &tasks.AgentExecutionConfig{}

	result.Model = input.Model
	result.Timeout = input.Timeout
	result.Temperature = input.Temperature
	if input.ContextManagement != nil {
		v, err := input.ContextManagement.toProto()
		if err != nil {
			return nil, err
		}
		result.ContextManagement = v
	}
	return result, nil
}

func (input *AgentCallTaskConfigInput) toProto() (*tasks.AgentCallTaskConfig, error) {
	result := &tasks.AgentCallTaskConfig{}

	result.Agent = input.Agent
	result.Org = input.Org
	result.Message = input.Message
	result.Env = input.Env
	if input.Config != nil {
		v, err := input.Config.toProto()
		if err != nil {
			return nil, err
		}
		result.Config = v
	}
	return result, nil
}

func (input *CallActivityTaskConfigInput) toProto() (*tasks.CallActivityTaskConfig, error) {
	result := &tasks.CallActivityTaskConfig{}

	result.Activity = input.Activity
	if len(input.Input) > 0 {
		v, err := structpb.NewStruct(input.Input)
		if err != nil {
			return nil, fmt.Errorf("marshal input: %w", err)
		}
		result.Input = v
	}
	return result, nil
}

func (input *ForTaskConfigInput) toProto() (*tasks.ForTaskConfig, error) {
	result := &tasks.ForTaskConfig{}

	result.Each = input.Each
	result.In = input.In
	for _, item := range input.Do {
		v, err := item.toProto()
		if err != nil {
			return nil, err
		}
		result.Do = append(result.Do, v)
	}
	return result, nil
}

func (input *ForkBranchInput) toProto() (*tasks.ForkBranch, error) {
	result := &tasks.ForkBranch{}

	result.Name = input.Name
	for _, item := range input.Do {
		v, err := item.toProto()
		if err != nil {
			return nil, err
		}
		result.Do = append(result.Do, v)
	}
	return result, nil
}

func (input *ForkTaskConfigInput) toProto() (*tasks.ForkTaskConfig, error) {
	result := &tasks.ForkTaskConfig{}

	for _, item := range input.Branches {
		v, err := item.toProto()
		if err != nil {
			return nil, err
		}
		result.Branches = append(result.Branches, v)
	}
	result.Compete = input.Compete
	return result, nil
}

func (input *GrpcCallTaskConfigInput) toProto() (*tasks.GrpcCallTaskConfig, error) {
	result := &tasks.GrpcCallTaskConfig{}

	result.Service = input.Service
	result.Method = input.Method
	if len(input.Request) > 0 {
		v, err := structpb.NewStruct(input.Request)
		if err != nil {
			return nil, fmt.Errorf("marshal request: %w", err)
		}
		result.Request = v
	}
	return result, nil
}

func (input *HttpEndpointInput) toProto() (*tasks.HttpEndpoint, error) {
	result := &tasks.HttpEndpoint{}

	result.Uri = input.Uri
	return result, nil
}

func (input *HttpCallTaskConfigInput) toProto() (*tasks.HttpCallTaskConfig, error) {
	result := &tasks.HttpCallTaskConfig{}

	result.Method = input.Method
	if input.Endpoint != nil {
		v, err := input.Endpoint.toProto()
		if err != nil {
			return nil, err
		}
		result.Endpoint = v
	}
	result.Headers = input.Headers
	if len(input.Body) > 0 {
		v, err := structpb.NewStruct(input.Body)
		if err != nil {
			return nil, fmt.Errorf("marshal body: %w", err)
		}
		result.Body = v
	}
	result.TimeoutSeconds = input.TimeoutSeconds
	return result, nil
}

func (input *SignalInput) toProto() (*tasks.SignalSpec, error) {
	result := &tasks.SignalSpec{}

	result.Id = input.Id
	result.Type = input.Type
	return result, nil
}

func (input *ListenToInput) toProto() (*tasks.ListenTo, error) {
	result := &tasks.ListenTo{}

	result.Mode = input.Mode
	for _, item := range input.Signals {
		v, err := item.toProto()
		if err != nil {
			return nil, err
		}
		result.Signals = append(result.Signals, v)
	}
	return result, nil
}

func (input *ListenTaskConfigInput) toProto() (*tasks.ListenTaskConfig, error) {
	result := &tasks.ListenTaskConfig{}

	if input.To != nil {
		v, err := input.To.toProto()
		if err != nil {
			return nil, err
		}
		result.To = v
	}
	return result, nil
}

func (input *RaiseTaskConfigInput) toProto() (*tasks.RaiseTaskConfig, error) {
	result := &tasks.RaiseTaskConfig{}

	result.Error = input.Error
	result.Message = input.Message
	return result, nil
}

func (input *RunTaskConfigInput) toProto() (*tasks.RunTaskConfig, error) {
	result := &tasks.RunTaskConfig{}

	result.Workflow = input.Workflow
	if len(input.Input) > 0 {
		v, err := structpb.NewStruct(input.Input)
		if err != nil {
			return nil, fmt.Errorf("marshal input: %w", err)
		}
		result.Input = v
	}
	return result, nil
}

func (input *SetTaskConfigInput) toProto() (*tasks.SetTaskConfig, error) {
	result := &tasks.SetTaskConfig{}

	result.Variables = input.Variables
	return result, nil
}

func (input *SwitchCaseInput) toProto() (*tasks.SwitchCase, error) {
	result := &tasks.SwitchCase{}

	result.Name = input.Name
	result.When = input.When
	result.Then = input.Then
	return result, nil
}

func (input *SwitchTaskConfigInput) toProto() (*tasks.SwitchTaskConfig, error) {
	result := &tasks.SwitchTaskConfig{}

	for _, item := range input.Cases {
		v, err := item.toProto()
		if err != nil {
			return nil, err
		}
		result.Cases = append(result.Cases, v)
	}
	return result, nil
}

func (input *CatchBlockInput) toProto() (*tasks.CatchBlock, error) {
	result := &tasks.CatchBlock{}

	result.As = input.As
	for _, item := range input.Do {
		v, err := item.toProto()
		if err != nil {
			return nil, err
		}
		result.Do = append(result.Do, v)
	}
	return result, nil
}

func (input *TryTaskConfigInput) toProto() (*tasks.TryTaskConfig, error) {
	result := &tasks.TryTaskConfig{}

	for _, item := range input.Try {
		v, err := item.toProto()
		if err != nil {
			return nil, err
		}
		result.Try = append(result.Try, v)
	}
	if input.Catch != nil {
		v, err := input.Catch.toProto()
		if err != nil {
			return nil, err
		}
		result.Catch = v
	}
	return result, nil
}

func (input *DurationInput) toProto() (*tasks.Duration, error) {
	result := &tasks.Duration{}

	result.Days = input.Days
	result.Hours = input.Hours
	result.Minutes = input.Minutes
	result.Seconds = input.Seconds
	result.Milliseconds = input.Milliseconds
	return result, nil
}

func (input *WaitTaskConfigInput) toProto() (*tasks.WaitTaskConfig, error) {
	result := &tasks.WaitTaskConfig{}

	if input.Duration != nil {
		v, err := input.Duration.toProto()
		if err != nil {
			return nil, err
		}
		result.WaitType = &tasks.WaitTaskConfig_Duration{Duration: v}
	}
	if input.Until != "" {
		t, err := time.Parse(time.RFC3339, input.Until)
		if err != nil {
			return nil, fmt.Errorf("parse until: %w", err)
		}
		result.WaitType = &tasks.WaitTaskConfig_Until{Until: timestamppb.New(t)}
	}
	return result, nil
}

func (input *ExportInput) toProto() (*workflowv1.Export, error) {
	result := &workflowv1.Export{}

	result.As = input.As
	return result, nil
}

func (input *FlowControlInput) toProto() (*workflowv1.FlowControl, error) {
	result := &workflowv1.FlowControl{}

	result.Then = input.Then
	return result, nil
}

func (input *WorkflowTaskInput) toProto() (*workflowv1.WorkflowTask, error) {
	result := &workflowv1.WorkflowTask{}

	result.Name = input.Name
	result.Kind = workflowv1.WorkflowTaskKind(workflowv1.WorkflowTaskKind_value[input.Kind])
	if input.Export != nil {
		v, err := input.Export.toProto()
		if err != nil {
			return nil, err
		}
		result.Export = v
	}
	if input.Flow != nil {
		v, err := input.Flow.toProto()
		if err != nil {
			return nil, err
		}
		result.Flow = v
	}
	switch input.Kind {
	case "agent_call":
		if input.AgentCall == nil {
			return nil, fmt.Errorf("agent_call config required when kind=%q", input.Kind)
		}
		p, err := input.AgentCall.toProto()
		if err != nil {
			return nil, fmt.Errorf("convert agent_call config: %w", err)
		}
		v, err := protoToStruct(p)
		if err != nil {
			return nil, fmt.Errorf("marshal agent_call config: %w", err)
		}
		result.TaskConfig = v
	case "activity_call":
		if input.ActivityCall == nil {
			return nil, fmt.Errorf("activity_call config required when kind=%q", input.Kind)
		}
		p, err := input.ActivityCall.toProto()
		if err != nil {
			return nil, fmt.Errorf("convert activity_call config: %w", err)
		}
		v, err := protoToStruct(p)
		if err != nil {
			return nil, fmt.Errorf("marshal activity_call config: %w", err)
		}
		result.TaskConfig = v
	case "for_each":
		if input.ForEach == nil {
			return nil, fmt.Errorf("for_each config required when kind=%q", input.Kind)
		}
		p, err := input.ForEach.toProto()
		if err != nil {
			return nil, fmt.Errorf("convert for_each config: %w", err)
		}
		v, err := protoToStruct(p)
		if err != nil {
			return nil, fmt.Errorf("marshal for_each config: %w", err)
		}
		result.TaskConfig = v
	case "fork":
		if input.Fork == nil {
			return nil, fmt.Errorf("fork config required when kind=%q", input.Kind)
		}
		p, err := input.Fork.toProto()
		if err != nil {
			return nil, fmt.Errorf("convert fork config: %w", err)
		}
		v, err := protoToStruct(p)
		if err != nil {
			return nil, fmt.Errorf("marshal fork config: %w", err)
		}
		result.TaskConfig = v
	case "grpc_call":
		if input.GrpcCall == nil {
			return nil, fmt.Errorf("grpc_call config required when kind=%q", input.Kind)
		}
		p, err := input.GrpcCall.toProto()
		if err != nil {
			return nil, fmt.Errorf("convert grpc_call config: %w", err)
		}
		v, err := protoToStruct(p)
		if err != nil {
			return nil, fmt.Errorf("marshal grpc_call config: %w", err)
		}
		result.TaskConfig = v
	case "http_call":
		if input.HttpCall == nil {
			return nil, fmt.Errorf("http_call config required when kind=%q", input.Kind)
		}
		p, err := input.HttpCall.toProto()
		if err != nil {
			return nil, fmt.Errorf("convert http_call config: %w", err)
		}
		v, err := protoToStruct(p)
		if err != nil {
			return nil, fmt.Errorf("marshal http_call config: %w", err)
		}
		result.TaskConfig = v
	case "listen":
		if input.Listen == nil {
			return nil, fmt.Errorf("listen config required when kind=%q", input.Kind)
		}
		p, err := input.Listen.toProto()
		if err != nil {
			return nil, fmt.Errorf("convert listen config: %w", err)
		}
		v, err := protoToStruct(p)
		if err != nil {
			return nil, fmt.Errorf("marshal listen config: %w", err)
		}
		result.TaskConfig = v
	case "raise_error":
		if input.RaiseError == nil {
			return nil, fmt.Errorf("raise_error config required when kind=%q", input.Kind)
		}
		p, err := input.RaiseError.toProto()
		if err != nil {
			return nil, fmt.Errorf("convert raise_error config: %w", err)
		}
		v, err := protoToStruct(p)
		if err != nil {
			return nil, fmt.Errorf("marshal raise_error config: %w", err)
		}
		result.TaskConfig = v
	case "run_workflow":
		if input.RunWorkflow == nil {
			return nil, fmt.Errorf("run_workflow config required when kind=%q", input.Kind)
		}
		p, err := input.RunWorkflow.toProto()
		if err != nil {
			return nil, fmt.Errorf("convert run_workflow config: %w", err)
		}
		v, err := protoToStruct(p)
		if err != nil {
			return nil, fmt.Errorf("marshal run_workflow config: %w", err)
		}
		result.TaskConfig = v
	case "set_vars":
		if input.SetVars == nil {
			return nil, fmt.Errorf("set_vars config required when kind=%q", input.Kind)
		}
		p, err := input.SetVars.toProto()
		if err != nil {
			return nil, fmt.Errorf("convert set_vars config: %w", err)
		}
		v, err := protoToStruct(p)
		if err != nil {
			return nil, fmt.Errorf("marshal set_vars config: %w", err)
		}
		result.TaskConfig = v
	case "switch_case":
		if input.SwitchCase == nil {
			return nil, fmt.Errorf("switch_case config required when kind=%q", input.Kind)
		}
		p, err := input.SwitchCase.toProto()
		if err != nil {
			return nil, fmt.Errorf("convert switch_case config: %w", err)
		}
		v, err := protoToStruct(p)
		if err != nil {
			return nil, fmt.Errorf("marshal switch_case config: %w", err)
		}
		result.TaskConfig = v
	case "try_catch":
		if input.TryCatch == nil {
			return nil, fmt.Errorf("try_catch config required when kind=%q", input.Kind)
		}
		p, err := input.TryCatch.toProto()
		if err != nil {
			return nil, fmt.Errorf("convert try_catch config: %w", err)
		}
		v, err := protoToStruct(p)
		if err != nil {
			return nil, fmt.Errorf("marshal try_catch config: %w", err)
		}
		result.TaskConfig = v
	case "wait":
		if input.Wait == nil {
			return nil, fmt.Errorf("wait config required when kind=%q", input.Kind)
		}
		p, err := input.Wait.toProto()
		if err != nil {
			return nil, fmt.Errorf("convert wait config: %w", err)
		}
		v, err := protoToStruct(p)
		if err != nil {
			return nil, fmt.Errorf("marshal wait config: %w", err)
		}
		result.TaskConfig = v
	default:
		if input.Kind != "" {
			return nil, fmt.Errorf("unknown task kind: %q", input.Kind)
		}
	}
	return result, nil
}

func (input *EnvironmentValue) toProto() (*environmentv1.EnvironmentValue, error) {
	result := &environmentv1.EnvironmentValue{}

	result.Value = input.Value
	result.IsSecret = input.IsSecret
	result.Description = input.Description
	return result, nil
}

func (input *EnvironmentInput) toProto() (*environmentv1.EnvironmentSpec, error) {
	result := &environmentv1.EnvironmentSpec{}

	result.Description = input.Description
	if len(input.Data) > 0 {
		result.Data = make(map[string]*environmentv1.EnvironmentValue, len(input.Data))
		for k, v := range input.Data {
			pv, err := v.toProto()
			if err != nil {
				return nil, err
			}
			result.Data[k] = pv
		}
	}
	return result, nil
}

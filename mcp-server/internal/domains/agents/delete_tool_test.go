package agents

import (
	"context"
	"encoding/json"
	"testing"

	"github.com/stigmer/stigmer/mcp-server/internal/auth"
	"github.com/stigmer/stigmer/mcp-server/internal/testutil"
	agentv1 "github.com/stigmer/stigmer/mcp-server/proto/ai/stigmer/agentic/agent/v1"
	"github.com/stigmer/stigmer/mcp-server/proto/ai/stigmer/commons/apiresource"
	"google.golang.org/grpc"
	"google.golang.org/grpc/codes"
	"google.golang.org/grpc/status"
)

// mockAgentDeleteController implements both Query and Command servers so that
// the fetch-then-delete flow can be tested end-to-end on a single gRPC server.
type mockAgentDeleteController struct {
	agentv1.UnimplementedAgentQueryControllerServer
	agentv1.UnimplementedAgentCommandControllerServer

	queryResp *agentv1.Agent
	queryErr  error

	gotDeleteID string
	deleteResp  *agentv1.Agent
	deleteErr   error
}

func (m *mockAgentDeleteController) GetByReference(_ context.Context, ref *apiresource.ApiResourceReference) (*agentv1.Agent, error) {
	return m.queryResp, m.queryErr
}

func (m *mockAgentDeleteController) Delete(_ context.Context, id *agentv1.AgentId) (*agentv1.Agent, error) {
	m.gotDeleteID = id.GetValue()
	return m.deleteResp, m.deleteErr
}

func TestDeleteTool_metadata(t *testing.T) {
	tool := DeleteTool()
	if tool.Name != "delete_agent" {
		t.Errorf("Name = %q, want %q", tool.Name, "delete_agent")
	}
	if tool.Description == "" {
		t.Error("Description is empty, want non-empty")
	}
}

func TestDeleteHandler_success(t *testing.T) {
	agent := &agentv1.Agent{
		ApiVersion: "agentic.stigmer.ai/v1",
		Kind:       "Agent",
		Metadata: &apiresource.ApiResourceMetadata{
			Org:  "acme",
			Slug: "code-reviewer",
			Name: "Code Reviewer",
			Id:   "agent-id-456",
		},
	}

	mock := &mockAgentDeleteController{
		queryResp:  agent,
		deleteResp: agent,
	}

	addr := testutil.StartGRPCServer(t, func(s *grpc.Server) {
		agentv1.RegisterAgentQueryControllerServer(s, mock)
		agentv1.RegisterAgentCommandControllerServer(s, mock)
	})

	ctx := auth.WithAPIKey(context.Background(), "test-key")
	handler := DeleteHandler(addr)

	result, _, err := handler(ctx, nil, &DeleteAgentInput{Org: "acme", Slug: "code-reviewer"})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}

	if mock.gotDeleteID != "agent-id-456" {
		t.Errorf("delete was called with ID %q, want %q", mock.gotDeleteID, "agent-id-456")
	}

	text := extractText(t, result)
	var raw map[string]any
	if err := json.Unmarshal([]byte(text), &raw); err != nil {
		t.Fatalf("response is not valid JSON: %v\ntext: %s", err, text)
	}
	if raw["api_version"] != "agentic.stigmer.ai/v1" {
		t.Errorf("api_version = %v, want %q", raw["api_version"], "agentic.stigmer.ai/v1")
	}
}

func TestDeleteHandler_notFound(t *testing.T) {
	mock := &mockAgentDeleteController{
		queryErr: status.Error(codes.NotFound, "agent not found"),
	}

	addr := testutil.StartGRPCServer(t, func(s *grpc.Server) {
		agentv1.RegisterAgentQueryControllerServer(s, mock)
		agentv1.RegisterAgentCommandControllerServer(s, mock)
	})

	ctx := auth.WithAPIKey(context.Background(), "test-key")
	handler := DeleteHandler(addr)

	_, _, err := handler(ctx, nil, &DeleteAgentInput{Org: "acme", Slug: "nonexistent"})
	if err == nil {
		t.Fatal("expected error for NotFound, got nil")
	}
}

func TestDeleteHandler_deletePermissionDenied(t *testing.T) {
	agent := &agentv1.Agent{
		Metadata: &apiresource.ApiResourceMetadata{Id: "agent-id-789"},
	}

	mock := &mockAgentDeleteController{
		queryResp: agent,
		deleteErr: status.Error(codes.PermissionDenied, "unauthorized to delete"),
	}

	addr := testutil.StartGRPCServer(t, func(s *grpc.Server) {
		agentv1.RegisterAgentQueryControllerServer(s, mock)
		agentv1.RegisterAgentCommandControllerServer(s, mock)
	})

	ctx := auth.WithAPIKey(context.Background(), "test-key")
	handler := DeleteHandler(addr)

	_, _, err := handler(ctx, nil, &DeleteAgentInput{Org: "acme", Slug: "restricted"})
	if err == nil {
		t.Fatal("expected error for PermissionDenied on delete, got nil")
	}
}

func TestDeleteHandler_missingAPIKey(t *testing.T) {
	handler := DeleteHandler("localhost:0")

	_, _, err := handler(context.Background(), nil, &DeleteAgentInput{Org: "acme", Slug: "x"})
	if err == nil {
		t.Fatal("expected error when API key is missing from context, got nil")
	}
}

...
// ApplyFunc is the signature shared by domain Apply functions that take a
// resource JSON string and return the result as JSON.
type ApplyFunc func(ctx context.Context, serverAddr, resourceJSON string) (string, error)
...
// CallApply calls fn and wraps its output in a CallToolResult. Use this in
// tool handlers for apply (create/update) operations that take a resource JSON.
func CallApply(fn ApplyFunc, ctx context.Context, serverAddr, resourceJSON string) (*mcp.CallToolResult, any, error) {
	text, err := fn(ctx, serverAddr, resourceJSON)
	if err != nil {
		return nil, nil, err
	}
	return TextResult(text)

...
// apply-capable resources into outputDir (mcp-server-ts/src/gen). It reuses the
...
func (m *mcpGen) generateTSFile() error {
	...
	imports.addValue("./apply-runtime", "generateSlug")
	imports.addValue("./apply-runtime", "visibilityFromString")
	...
	full.WriteString("// Go parity: mcp-server/gen/" + strings.TrimPrefix(tsProtoPkg(m.spec.ProtoType), "ai.stigmer.") + "\n\n")
	imports.emit(&full)
...
func (m *mcpGen) genTSFieldAssign(w *bytes.Buffer, tf tsField, dst string, imports *tsImportSet) {
	...
	case tf.enumType != "":
		...
		imports.addValue("./apply-runtime", "enumFromString")
		if tf.coll == tsArray {
	...
	case tf.isTimestamp:
		imports.addValue("./apply-runtime", "toTimestamp")
...
// Go parity: mcp-server/internal/convert/convert.go.

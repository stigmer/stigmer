...
// TestStripInternalSection pins the delegation to the shared convention
// package; the full semantics table lives in
// tools/codegen/internalcomment/internalcomment_test.go (moved there when
// oss#497 gave the convention a second consumer, stubscrub).
func TestStripInternalSection(t *testing.T) {
	got := stripInternalSection("Public text.\n\n@internal\nHandler strategy notes.")
	if got != "Public text." {
		t.Errorf("stripInternalSection delegation broken: got %q, want %q", got, "Public text.")
	}
}

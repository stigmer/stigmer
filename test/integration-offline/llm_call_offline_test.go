...
// TestOffline_LlmCall_OpenAI_StructuredOutput was removed on 2026-07-22: the
// native harness registry was trimmed to Anthropic-only (OpenAI + Ollama
// entries retired from the model pricing baseline), so a workflow pinning an
// OpenAI native model can no longer pass server-side model validation.

...
// pins only the capture gate:
//
// Memory create is guarded by the strict first-party-human-operator gate
// (DD-002 D4 as amended, inherited by capture — DD-005 D2). This
// harness has NO credential that passes it, structurally: the tokenless
// suite identity is deliberately machine-class
// (IntegrationTestSecurityConfig), and every mintable token is a
// PlatformClient user token — the exact credential class the owner
// amendment excludes. Seeding memory rows behind the service's back is
// off the table too (the IdentitySeeder doctrine: the pre-auth
// bootstrap is the ONLY legitimate direct Tier-1 write; everything else
// seeds through the front door).
//
// The full behavioral matrix therefore lives where it is reachable:
...
//
// What THIS file pins is the one thing only a wire test against the
// real service can: the gate actually refuses both excluded caller
// classes at the create RPC, with the cross-edition copy — the same
// posture as ComposeDeclaredPreferencesStep, whose eligible-caller path
// is likewise unit-covered because no harness credential passes the gate.
